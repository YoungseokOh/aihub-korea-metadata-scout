...
## Basic Info
- Dataset Key: `71680`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71680`
- Collected At: `2026-03-17T08:41:31.573072+00:00`
...
## File Overview
- File count: `4`
- Total known size: `11.0 GB`
- Representative file paths:
  - `024.아동·청소년 상담 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_in.zip`
  - `024.아동·청소년 상담 데이터/3.개방데이터/1.데이터/Training/02.라벨링데이터/TL_out.zip`
  - `024.아동·청소년 상담 데이터/3.개방데이터/1.데이터/Validation/01.원천데이터/VS_in.zip`
  - `024.아동·청소년 상담 데이터/3.개방데이터/1.데이터/Validation/02.라벨링데이터/VL_out.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `speech/dialogue` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 4개이며 알려진 총 용량은 11.0 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
