...
## Basic Info
- Dataset Key: `71721`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71721`
- Collected At: `2026-03-17T08:41:34.883095+00:00`
...
## File Overview
- File count: `14`
- Total known size: `958.1 GB`
- Representative file paths:
  - `035.한국형 텍스트-3D 객체 쌍 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS.z01`
  - `035.한국형 텍스트-3D 객체 쌍 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS.z02`
  - `035.한국형 텍스트-3D 객체 쌍 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS.z03`
  - `035.한국형 텍스트-3D 객체 쌍 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS.z04`
  - `035.한국형 텍스트-3D 객체 쌍 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS.z05`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `3d/lidar` 자료로 보입니다. 확인 가능한 파일은 14개이며 알려진 총 용량은 958.1 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 14,
  "total_size_bytes": 1028747427840,
  "human_size": "958.1 GB",
  "sample_file_paths": [
    "035.한국형 텍스트-3D 객체 쌍 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS.z01",
    "035.한국형 텍스트-3D 객체 쌍 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS.z02",
