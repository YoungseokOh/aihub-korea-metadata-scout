...
## Basic Info
- Dataset Key: `469`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 469`
- Collected At: `2026-03-17T08:40:41.456876+00:00`
...
- Representative file paths:
  - `034.마스크 착용 한국인 안면 이미지 데이터/01.데이터/1.Training/원천데이터/TS2.zip`
  - `034.마스크 착용 한국인 안면 이미지 데이터/01.데이터/1.Training/원천데이터/TS3.zip`
  - `034.마스크 착용 한국인 안면 이미지 데이터/01.데이터/1.Training/원천데이터/TS4.zip`
  - `034.마스크 착용 한국인 안면 이미지 데이터/01.데이터/1.Training/원천데이터/TS5.zip`
  - `034.마스크 착용 한국인 안면 이미지 데이터/01.데이터/1.Training/원천데이터/TS6.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 12개이며 알려진 총 용량은 1.16 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 12,
  "total_size_bytes": 1275806613504,
  "human_size": "1.16 TB",
  "sample_file_paths": [
    "034.마스크 착용 한국인 안면 이미지 데이터/01.데이터/1.Training/원천데이터/TS2.zip",
    "034.마스크 착용 한국인 안면 이미지 데이터/01.데이터/1.Training/원천데이터/TS3.zip",
