...
## Basic Info
- Dataset Key: `527`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 527`
- Collected At: `2026-03-17T08:40:45.678370+00:00`
...
## File Overview
- File count: `56`
- Total known size: `481.5 GB`
- Representative file paths:
  - `102.정밀농업 농기계 잡초 인식 데이터/01.데이터/2.Validation/원천데이터/VS3_개여뀌.zip`
  - `102.정밀농업 농기계 잡초 인식 데이터/01.데이터/2.Validation/원천데이터/VS4_깨풀.zip`
  - `102.정밀농업 농기계 잡초 인식 데이터/01.데이터/2.Validation/원천데이터/VS5_닭의장풀.zip`
  - `102.정밀농업 농기계 잡초 인식 데이터/01.데이터/2.Validation/원천데이터/VS6_물달개비.zip`
  - `102.정밀농업 농기계 잡초 인식 데이터/01.데이터/2.Validation/원천데이터/VS7_미국가막사리.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 56개이며 알려진 총 용량은 481.5 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
