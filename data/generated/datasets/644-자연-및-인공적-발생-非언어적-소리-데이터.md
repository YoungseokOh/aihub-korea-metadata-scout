...
## Basic Info
- Dataset Key: `644`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 644`
- Collected At: `2026-03-17T08:40:55.639268+00:00`
...
- Representative file paths:
  - `174.자연 및 인공적 발생 非언어적 소리 데이터/01.데이터/2.Validation/원천데이터/VS_8.군부대 운송수단.zip`
  - `174.자연 및 인공적 발생 非언어적 소리 데이터/01.데이터/2.Validation/원천데이터/VS_9.생활.zip`
  - `174.자연 및 인공적 발생 非언어적 소리 데이터/01.데이터/2.Validation/원천데이터/VS_1.자연.zip`
  - `174.자연 및 인공적 발생 非언어적 소리 데이터/01.데이터/2.Validation/원천데이터/VS_10.기계 및 공구.zip`
  - `174.자연 및 인공적 발생 非언어적 소리 데이터/01.데이터/2.Validation/원천데이터/VS_2.무기.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 40개이며 알려진 총 용량은 18.3 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
