# 다중밀집시설 및 주거시설 화재 안전 데이터

## Basic Info
- Dataset Key: `518`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 518`
- Collected At: `2026-03-17T08:40:44.925121+00:00`
...
## File Overview
- File count: `100`
- Total known size: `961.2 GB`
- Representative file paths:
  - `115.다중밀집시설 및 주거시설 화재 안전 데이터/01.데이터/2.Validation/라벨링데이터/VL23.zip`
  - `115.다중밀집시설 및 주거시설 화재 안전 데이터/01.데이터/2.Validation/라벨링데이터/VL24.zip`
  - `115.다중밀집시설 및 주거시설 화재 안전 데이터/01.데이터/2.Validation/라벨링데이터/VL25.zip`
  - `115.다중밀집시설 및 주거시설 화재 안전 데이터/01.데이터/2.Validation/라벨링데이터/VL3.zip`
  - `115.다중밀집시설 및 주거시설 화재 안전 데이터/01.데이터/2.Validation/라벨링데이터/VL4.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 100개이며 알려진 총 용량은 961.2 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "modality_guess": null,
  "file_count": 100,
  "total_size_bytes": 1032099085312,
  "human_size": "961.2 GB",
  "sample_file_paths": [
    "115.다중밀집시설 및 주거시설 화재 안전 데이터/01.데이터/2.Validation/라벨링데이터/VL23.zip",
    "115.다중밀집시설 및 주거시설 화재 안전 데이터/01.데이터/2.Validation/라벨링데이터/VL24.zip",
