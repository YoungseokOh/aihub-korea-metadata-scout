...
## Basic Info
- Dataset Key: `55`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 55`
- Collected At: `2026-03-17T08:40:21.169943+00:00`
...
## File Overview
- File count: `102`
- Total known size: `2.73 TB`
- Representative file paths:
  - `003.딥페이크/2.Validation/원천데이터/validate_탐지방해/원본영상/02_validate.zip`
  - `003.딥페이크/2.Validation/원천데이터/validate_탐지방해/원본영상/03_validate.zip`
  - `003.딥페이크/2.Validation/원천데이터/validate_탐지방해/원본영상/01_validate.zip`
  - `003.딥페이크/2.Validation/원천데이터/validate_탐지방해/변조영상/01_validate_탐지방해.zip`
  - `003.딥페이크/2.Validation/원천데이터/validate_탐지방해/변조영상/02_validate_탐지방해.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 102개이며 알려진 총 용량은 2.73 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 102,
  "total_size_bytes": 3006478191616,
  "human_size": "2.73 TB",
  "sample_file_paths": [
    "003.딥페이크/2.Validation/원천데이터/validate_탐지방해/원본영상/02_validate.zip",
    "003.딥페이크/2.Validation/원천데이터/validate_탐지방해/원본영상/03_validate.zip",
