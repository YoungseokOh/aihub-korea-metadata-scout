...
## Basic Info
- Dataset Key: `71795`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71795`
- Collected At: `2026-03-17T08:41:40.842250+00:00`
...
## File Overview
- File count: `21`
- Total known size: `323.5 MB`
- Representative file paths:
  - `023.국회 회의록 기반 지식검색 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_국정감사.zip`
  - `023.국회 회의록 기반 지식검색 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_본회의.zip`
  - `023.국회 회의록 기반 지식검색 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_소위원회.zip`
  - `023.국회 회의록 기반 지식검색 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_예산결산특별위원회.zip`
  - `023.국회 회의록 기반 지식검색 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_특별위원회.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `nlp/knowledge` 성격의 `text` 자료로 보입니다. 확인 가능한 파일은 21개이며 알려진 총 용량은 323.5 MB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
