...
## Basic Info
- Dataset Key: `71673`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71673`
- Collected At: `2026-03-17T08:41:31.135665+00:00`
...
## File Overview
- File count: `85`
- Total known size: `165.0 GB`
- Representative file paths:
  - `072.해파리 초기유생 데이터/3.개방데이터/1.데이터/Other/Other.zip`
  - `072.해파리 초기유생 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_01. 자연채집_경기.zip`
  - `072.해파리 초기유생 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_01. 자연채집_경남.zip`
  - `072.해파리 초기유생 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_01. 자연채집_경북 강원.zip`
  - `072.해파리 초기유생 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_01. 자연채집_전남.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 85개이며 알려진 총 용량은 165.0 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
