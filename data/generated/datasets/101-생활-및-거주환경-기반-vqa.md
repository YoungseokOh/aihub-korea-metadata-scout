...
## Basic Info
- Dataset Key: `101`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 101`
- Collected At: `2026-03-17T08:40:26.996206+00:00`
...
## File Overview
- File count: `18`
- Total known size: `135.2 GB`
- Representative file paths:
  - `126.생활 및 거주환경 VQA/01.데이터/1.Training/라벨링데이터/개인생활환경 이미지 수집.zip`
  - `126.생활 및 거주환경 VQA/01.데이터/1.Training/라벨링데이터/노인 생활거주환경 이미지 수집.zip`
  - `126.생활 및 거주환경 VQA/01.데이터/1.Training/라벨링데이터/실내 가전 및 가구배치 이미지 수집_1.zip`
  - `126.생활 및 거주환경 VQA/01.데이터/1.Training/라벨링데이터/실내 가전 및 가구배치 이미지 수집_2.zip`
  - `126.생활 및 거주환경 VQA/01.데이터/1.Training/라벨링데이터/어린이 생활거주환경 이미지 수집.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `multimodal` 자료로 보입니다. 확인 가능한 파일은 18개이며 알려진 총 용량은 135.2 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
