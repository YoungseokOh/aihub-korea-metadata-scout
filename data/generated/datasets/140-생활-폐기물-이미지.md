...
## Basic Info
- Dataset Key: `140`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 140`
- Collected At: `2026-03-17T08:40:31.225774+00:00`
...
## File Overview
- File count: `254`
- Total known size: `333.6 GB`
- Representative file paths:
  - `085.생활 폐기물 이미지/01.데이터/1.Training/라벨링데이터/라벨링데이터.zip`
  - `085.생활 폐기물 이미지/01.데이터/1.Training/원천데이터/가구류/밥상/밥상.zip`
  - `085.생활 폐기물 이미지/01.데이터/1.Training/원천데이터/가구류/서랍장/서랍장.zip`
  - `085.생활 폐기물 이미지/01.데이터/1.Training/원천데이터/가구류/소파/소파.zip`
  - `085.생활 폐기물 이미지/01.데이터/1.Training/원천데이터/가구류/수납장/수납장.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 254개이며 알려진 총 용량은 333.6 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
