...
## Basic Info
- Dataset Key: `71647`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71647`
- Collected At: `2026-03-17T08:41:30.416192+00:00`
...
## File Overview
- File count: `18`
- Total known size: `1.44 TB`
- Representative file paths:
  - `029.물체 조작 손 동작 3D 데이터/3.개방데이터/1.데이터/Other/Other.zip`
  - `029.물체 조작 손 동작 3D 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS.z01`
  - `029.물체 조작 손 동작 3D 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS.z02`
  - `029.물체 조작 손 동작 3D 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS.z03`
  - `029.물체 조작 손 동작 3D 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS.z04`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `3d/lidar` 자료로 보입니다. 확인 가능한 파일은 18개이며 알려진 총 용량은 1.44 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 18,
  "total_size_bytes": 1587063816192,
  "human_size": "1.44 TB",
  "sample_file_paths": [
    "029.물체 조작 손 동작 3D 데이터/3.개방데이터/1.데이터/Other/Other.zip",
    "029.물체 조작 손 동작 3D 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS.z01",
