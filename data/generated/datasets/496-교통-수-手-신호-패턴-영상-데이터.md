...
## Basic Info
- Dataset Key: `496`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 496`
- Collected At: `2026-03-17T08:40:43.644740+00:00`
...
## File Overview
- File count: `55`
- Total known size: `2.37 TB`
- Representative file paths:
  - `059.교통 수(手)신호 패턴 영상 데이터/01.데이터/1.Training/원천데이터/TS26.zip`
  - `059.교통 수(手)신호 패턴 영상 데이터/01.데이터/1.Training/원천데이터/TS27.zip`
  - `059.교통 수(手)신호 패턴 영상 데이터/01.데이터/1.Training/원천데이터/TS28.zip`
  - `059.교통 수(手)신호 패턴 영상 데이터/01.데이터/1.Training/원천데이터/TS29.zip`
  - `059.교통 수(手)신호 패턴 영상 데이터/01.데이터/1.Training/원천데이터/TS3.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 55개이며 알려진 총 용량은 2.37 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 55,
  "total_size_bytes": 2607234940928,
  "human_size": "2.37 TB",
  "sample_file_paths": [
    "059.교통 수(手)신호 패턴 영상 데이터/01.데이터/1.Training/원천데이터/TS26.zip",
    "059.교통 수(手)신호 패턴 영상 데이터/01.데이터/1.Training/원천데이터/TS27.zip",
