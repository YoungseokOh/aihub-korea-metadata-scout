...
## Basic Info
- Dataset Key: `174`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 174`
- Collected At: `2026-03-17T08:40:34.859157+00:00`
...
## File Overview
- File count: `246`
- Total known size: `1.67 TB`
- Representative file paths:
  - `107.CCTV 이상행동 영상 데이터/01.데이터/1.Training/원천데이터/실신/실신_6.zip`
  - `107.CCTV 이상행동 영상 데이터/01.데이터/1.Training/원천데이터/실신/실신_7.zip`
  - `107.CCTV 이상행동 영상 데이터/01.데이터/1.Training/원천데이터/실신/실신_8.zip`
  - `107.CCTV 이상행동 영상 데이터/01.데이터/1.Training/원천데이터/실신/실신_1.zip`
  - `107.CCTV 이상행동 영상 데이터/01.데이터/1.Training/원천데이터/실신/실신_2.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 246개이며 알려진 총 용량은 1.67 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 246,
  "total_size_bytes": 1837892608000,
  "human_size": "1.67 TB",
  "sample_file_paths": [
    "107.CCTV 이상행동 영상 데이터/01.데이터/1.Training/원천데이터/실신/실신_6.zip",
    "107.CCTV 이상행동 영상 데이터/01.데이터/1.Training/원천데이터/실신/실신_7.zip",
