...
## Basic Info
- Dataset Key: `71700`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71700`
- Collected At: `2026-03-17T08:41:33.057770+00:00`
...
- Representative file paths:
  - `063.과수원 내 로봇 주행 데이터_사과, 배 류/3.개방데이터/1.데이터/Other/Other.z01`
  - `063.과수원 내 로봇 주행 데이터_사과, 배 류/3.개방데이터/1.데이터/Other/Other.z02`
  - `063.과수원 내 로봇 주행 데이터_사과, 배 류/3.개방데이터/1.데이터/Other/Other.z03`
  - `063.과수원 내 로봇 주행 데이터_사과, 배 류/3.개방데이터/1.데이터/Other/Other.z04`
  - `063.과수원 내 로봇 주행 데이터_사과, 배 류/3.개방데이터/1.데이터/Other/Other.z05`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `robotics` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 52개이며 알려진 총 용량은 4.68 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "modality_guess": null,
  "file_count": 52,
  "total_size_bytes": 5145258622976,
  "human_size": "4.68 TB",
  "sample_file_paths": [
    "063.과수원 내 로봇 주행 데이터_사과, 배 류/3.개방데이터/1.데이터/Other/Other.z01",
    "063.과수원 내 로봇 주행 데이터_사과, 배 류/3.개방데이터/1.데이터/Other/Other.z02",
