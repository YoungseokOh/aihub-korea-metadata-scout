...
## Basic Info
- Dataset Key: `111`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 111`
- Collected At: `2026-03-17T08:40:28.938816+00:00`
...
## File Overview
- File count: `4`
- Total known size: `407.0 MB`
- Representative file paths:
  - `006.전문분야한영/1.Training/라벨링데이터/ko2en_training_json.zip`
  - `006.전문분야한영/1.Training/원천데이터/ko2en_training_csv.zip`
  - `006.전문분야한영/2.Validation/라벨링데이터/ko2en_validation_json.zip`
  - `006.전문분야한영/2.Validation/원천데이터/ko2en_validation_csv.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `nlp/knowledge` 성격의 `text` 자료로 보입니다. 확인 가능한 파일은 4개이며 알려진 총 용량은 407.0 MB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
