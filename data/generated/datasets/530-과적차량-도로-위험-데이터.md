...
## Basic Info
- Dataset Key: `530`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 530`
- Collected At: `2026-03-17T08:40:45.906356+00:00`
...
## File Overview
- File count: `12`
- Total known size: `159.3 GB`
- Representative file paths:
  - `118.과적차량 도로 위험 데이터/01.데이터/01.데이터/1.Training/라벨링데이터/TL3.소형차.zip`
  - `118.과적차량 도로 위험 데이터/01.데이터/01.데이터/1.Training/라벨링데이터/TL1.대형차.zip`
  - `118.과적차량 도로 위험 데이터/01.데이터/01.데이터/1.Training/라벨링데이터/TL2.중형차.zip`
  - `118.과적차량 도로 위험 데이터/01.데이터/01.데이터/1.Training/원천데이터/TS1.대형차.zip`
  - `118.과적차량 도로 위험 데이터/01.데이터/01.데이터/1.Training/원천데이터/TS2.중형차.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 12개이며 알려진 총 용량은 159.3 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
