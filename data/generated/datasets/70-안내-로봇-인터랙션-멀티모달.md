...
## Basic Info
- Dataset Key: `70`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 70`
- Collected At: `2026-03-17T08:40:22.781305+00:00`
...
## File Overview
- File count: `96`
- Total known size: `2.38 TB`
- Representative file paths:
  - `136.안내 로봇 인터랙션/01.데이터/2.Validation/라벨링데이터(zip)/여(성별)-중간(촬영방향).zip`
  - `136.안내 로봇 인터랙션/01.데이터/2.Validation/라벨링데이터(zip)/남(성별)-뒤(촬영방향).zip`
  - `136.안내 로봇 인터랙션/01.데이터/2.Validation/라벨링데이터(zip)/남(성별)-아래(촬영방향).zip`
  - `136.안내 로봇 인터랙션/01.데이터/2.Validation/라벨링데이터(zip)/남(성별)-우(촬영방향).zip`
  - `136.안내 로봇 인터랙션/01.데이터/2.Validation/라벨링데이터(zip)/남(성별)-위(촬영방향).zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `robotics` 성격의 `multimodal` 자료로 보입니다. 확인 가능한 파일은 96개이며 알려진 총 용량은 2.38 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 96,
  "total_size_bytes": 2612927660032,
  "human_size": "2.38 TB",
  "sample_file_paths": [
    "136.안내 로봇 인터랙션/01.데이터/2.Validation/라벨링데이터(zip)/여(성별)-중간(촬영방향).zip",
    "136.안내 로봇 인터랙션/01.데이터/2.Validation/라벨링데이터(zip)/남(성별)-뒤(촬영방향).zip",
