...
## Basic Info
- Dataset Key: `71388`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71388`
- Collected At: `2026-03-17T08:41:08.155618+00:00`
...
## File Overview
- File count: `168`
- Total known size: `413.3 GB`
- Representative file paths:
  - `237.건설 현장 장비 모니터링 및 생산성 측정 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_위치궤적_piledrivera.zip`
  - `237.건설 현장 장비 모니터링 및 생산성 측정 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_위치궤적_piledriverb.zip`
  - `237.건설 현장 장비 모니터링 및 생산성 측정 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_위치궤적_piledriverc.zip`
  - `237.건설 현장 장비 모니터링 및 생산성 측정 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_위치궤적_pumpcar.zip`
  - `237.건설 현장 장비 모니터링 및 생산성 측정 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_위치궤적_trailer.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 168개이며 알려진 총 용량은 413.3 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
{
  "dataset_key": 71388,
  "title": "건설 현장 장비 모니터링 및 생산성 측정 데이터",
  "tags": [
    "computer-vision",
    "image",
    "이미지",
