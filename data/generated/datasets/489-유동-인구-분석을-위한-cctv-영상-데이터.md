# 유동 인구 분석을 위한 CCTV 영상 데이터

## Basic Info
- Dataset Key: `489`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 489`
- Collected At: `2026-03-17T08:40:42.996221+00:00`
...
## File Overview
- File count: `36`
- Total known size: `377.2 GB`
- Representative file paths:
  - `154.유동 인구 분석을 위한 CCTV 영상 데이터/01.데이터/1.Training/라벨링데이터_1128_add/TL11_ye-ma.zip`
  - `154.유동 인구 분석을 위한 CCTV 영상 데이터/01.데이터/1.Training/라벨링데이터_1107_add/TL10_se-yk.zip`
  - `154.유동 인구 분석을 위한 CCTV 영상 데이터/01.데이터/1.Training/라벨링데이터_1107_add/TL1_do-sa.zip`
  - `154.유동 인구 분석을 위한 CCTV 영상 데이터/01.데이터/1.Training/라벨링데이터_1107_add/TL2_ja-ma.zip`
  - `154.유동 인구 분석을 위한 CCTV 영상 데이터/01.데이터/1.Training/라벨링데이터_1107_add/TL3_ju-ja_1.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 36개이며 알려진 총 용량은 377.2 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
{
  "dataset_key": 489,
  "title": "유동 인구 분석을 위한 CCTV 영상 데이터",
  "tags": [
    "computer-vision",
    "video",
    "영상",
