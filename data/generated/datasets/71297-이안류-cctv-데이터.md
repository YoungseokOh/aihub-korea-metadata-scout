...
## Basic Info
- Dataset Key: `71297`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71297`
- Collected At: `2026-03-17T08:40:58.985053+00:00`
...
## File Overview
- File count: `58`
- Total known size: `31.1 GB`
- Representative file paths:
  - `290.이안류 CCTV 데이터/01-1.정식개방데이터/Validation/01.원천데이터/VS_1.이미지_1.해운대_3.PARA2.zip`
  - `290.이안류 CCTV 데이터/01-1.정식개방데이터/Validation/01.원천데이터/VS_1.이미지_1.해운대_1.GLORY.zip`
  - `290.이안류 CCTV 데이터/01-1.정식개방데이터/Validation/01.원천데이터/VS_1.이미지_1.해운대_2.PARA1.zip`
  - `290.이안류 CCTV 데이터/01-1.정식개방데이터/Validation/01.원천데이터/VS_1.이미지_4.대천_2.ZIPTR.zip`
  - `290.이안류 CCTV 데이터/01-1.정식개방데이터/Validation/01.원천데이터/VS_1.이미지_5.낙산_1.NSBE1.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 58개이며 알려진 총 용량은 31.1 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
