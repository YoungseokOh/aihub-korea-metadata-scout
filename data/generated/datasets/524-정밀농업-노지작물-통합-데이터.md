...
## Basic Info
- Dataset Key: `524`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 524`
- Collected At: `2026-03-17T08:40:45.385344+00:00`
...
## File Overview
- File count: `50`
- Total known size: `980.9 GB`
- Representative file paths:
  - `101.정밀농업 노지작물 통합 데이터/01.데이터/1.Training/라벨링데이터/TL4_들깨_2.zip`
  - `101.정밀농업 노지작물 통합 데이터/01.데이터/1.Training/라벨링데이터/TL5_벼_조생종.zip`
  - `101.정밀농업 노지작물 통합 데이터/01.데이터/1.Training/라벨링데이터/TL6_벼_중만생종.zip`
  - `101.정밀농업 노지작물 통합 데이터/01.데이터/1.Training/라벨링데이터/TL7_옥수수_광평옥_1.zip`
  - `101.정밀농업 노지작물 통합 데이터/01.데이터/1.Training/라벨링데이터/TL8_옥수수_광평옥_2.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 50개이며 알려진 총 용량은 980.9 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "modality_guess": null,
  "file_count": 50,
  "total_size_bytes": 1053202317312,
  "human_size": "980.9 GB",
  "sample_file_paths": [
    "101.정밀농업 노지작물 통합 데이터/01.데이터/1.Training/라벨링데이터/TL4_들깨_2.zip",
    "101.정밀농업 노지작물 통합 데이터/01.데이터/1.Training/라벨링데이터/TL5_벼_조생종.zip",
