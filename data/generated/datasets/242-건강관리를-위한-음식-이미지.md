...
## Basic Info
- Dataset Key: `242`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 242`
- Collected At: `2026-03-17T08:40:39.786060+00:00`
...
## File Overview
- File count: `105`
- Total known size: `842.4 GB`
- Representative file paths:
  - `052.건강관리를 위한 음식 이미지 데이터/01.데이터/1.Training/라벨링데이터/음식110_Tra_json.zip`
  - `052.건강관리를 위한 음식 이미지 데이터/01.데이터/1.Training/라벨링데이터/음식203_Tra_json.zip`
  - `052.건강관리를 위한 음식 이미지 데이터/01.데이터/1.Training/라벨링데이터/음식204_Tra_json.zip`
  - `052.건강관리를 위한 음식 이미지 데이터/01.데이터/1.Training/라벨링데이터/음식205_Tra_json.zip`
  - `052.건강관리를 위한 음식 이미지 데이터/01.데이터/1.Training/라벨링데이터/음식206_Tra_json.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 105개이며 알려진 총 용량은 842.4 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
