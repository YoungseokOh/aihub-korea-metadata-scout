...
## Basic Info
- Dataset Key: `71688`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71688`
- Collected At: `2026-03-17T08:41:32.093252+00:00`
...
## File Overview
- File count: `21`
- Total known size: `215.9 GB`
- Representative file paths:
  - `052.구강 이미지 합성데이터/3.개방데이터/1.데이터/Training/1.원천데이터/1.right.zip`
  - `052.구강 이미지 합성데이터/3.개방데이터/1.데이터/Training/1.원천데이터/2.front.zip`
  - `052.구강 이미지 합성데이터/3.개방데이터/1.데이터/Training/1.원천데이터/3.left.zip`
  - `052.구강 이미지 합성데이터/3.개방데이터/1.데이터/Training/1.원천데이터/4.upper.zip`
  - `052.구강 이미지 합성데이터/3.개방데이터/1.데이터/Training/1.원천데이터/5.lower.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 21개이며 알려진 총 용량은 215.9 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
