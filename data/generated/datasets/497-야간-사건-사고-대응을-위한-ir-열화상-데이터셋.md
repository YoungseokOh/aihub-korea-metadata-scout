...
## Basic Info
- Dataset Key: `497`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 497`
- Collected At: `2026-03-17T08:40:43.727031+00:00`
...
## File Overview
- File count: `36`
- Total known size: `433.3 GB`
- Representative file paths:
  - `136.야간 사건사고 대응을 위한 버드아이뷰 IR열화상 데이터/01.데이터/1.Training/라벨링데이터/TL_객체데이터이미지_5.zip`
  - `136.야간 사건사고 대응을 위한 버드아이뷰 IR열화상 데이터/01.데이터/1.Training/라벨링데이터/TL_객체데이터이미지_6.zip`
  - `136.야간 사건사고 대응을 위한 버드아이뷰 IR열화상 데이터/01.데이터/1.Training/라벨링데이터/TL_사건사고데이터이미지_0.zip`
  - `136.야간 사건사고 대응을 위한 버드아이뷰 IR열화상 데이터/01.데이터/1.Training/라벨링데이터/TL_사건사고데이터이미지_1.zip`
  - `136.야간 사건사고 대응을 위한 버드아이뷰 IR열화상 데이터/01.데이터/1.Training/라벨링데이터/TL_사건사고데이터이미지_2.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 36개이며 알려진 총 용량은 433.3 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
{
  "dataset_key": 497,
  "title": "야간 사건 사고 대응을 위한 IR 열화상 데이터셋",
  "tags": [
    "computer-vision",
    "image",
    "객체",
