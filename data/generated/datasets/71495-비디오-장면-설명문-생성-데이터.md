...
## Basic Info
- Dataset Key: `71495`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71495`
- Collected At: `2026-03-17T08:41:17.563918+00:00`
...
## File Overview
- File count: `401`
- Total known size: `6.88 TB`
- Representative file paths:
  - `003.비디오 장면 설명문 생성 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_드라마_220804.zip`
  - `003.비디오 장면 설명문 생성 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_드라마_220816.zip`
  - `003.비디오 장면 설명문 생성 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_드라마_220817.zip`
  - `003.비디오 장면 설명문 생성 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_드라마_220822.zip`
  - `003.비디오 장면 설명문 생성 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_드라마_220824.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 401개이며 알려진 총 용량은 6.88 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 401,
  "total_size_bytes": 7570079569920,
  "human_size": "6.88 TB",
  "sample_file_paths": [
    "003.비디오 장면 설명문 생성 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_드라마_220804.zip",
    "003.비디오 장면 설명문 생성 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_드라마_220816.zip",
