...
## Basic Info
- Dataset Key: `169`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 169`
- Collected At: `2026-03-17T08:40:34.299741+00:00`
...
## File Overview
- File count: `591`
- Total known size: `5.94 TB`
- Representative file paths:
  - `110.어린이 보호구역 내 어린이 도로보행 위험행동 영상 데이터/01.데이터/1.Training/라벨링데이터/1.CCTV/jay_walk_8.zip`
  - `110.어린이 보호구역 내 어린이 도로보행 위험행동 영상 데이터/01.데이터/1.Training/라벨링데이터/1.CCTV/putup_umbrella_1.zip`
  - `110.어린이 보호구역 내 어린이 도로보행 위험행동 영상 데이터/01.데이터/1.Training/라벨링데이터/1.CCTV/putup_umbrella_2.zip`
  - `110.어린이 보호구역 내 어린이 도로보행 위험행동 영상 데이터/01.데이터/1.Training/라벨링데이터/1.CCTV/ride_cycle_1.zip`
  - `110.어린이 보호구역 내 어린이 도로보행 위험행동 영상 데이터/01.데이터/1.Training/라벨링데이터/1.CCTV/ride_cycle_2.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 591개이며 알려진 총 용량은 5.94 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
{
  "dataset_key": 169,
  "title": "어린이 보호구역 내 어린이 도로보행 위험행동 영상",
  "tags": [
    "computer-vision",
    "video",
    "image",
  ...
  "file_count": 591,
  "total_size_bytes": 6533915130880,
  "human_size": "5.94 TB",
  "sample_file_paths": [
    "110.어린이 보호구역 내 어린이 도로보행 위험행동 영상 데이터/01.데이터/1.Training/라벨링데이터/1.CCTV/jay_walk_8.zip",
    "110.어린이 보호구역 내 어린이 도로보행 위험행동 영상 데이터/01.데이터/1.Training/라벨링데이터/1.CCTV/putup_umbrella_1.zip",
