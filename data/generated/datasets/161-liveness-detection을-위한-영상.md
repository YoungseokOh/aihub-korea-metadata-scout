...
## Basic Info
- Dataset Key: `161`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 161`
- Collected At: `2026-03-17T08:40:33.305811+00:00`
...
## File Overview
- File count: `33`
- Total known size: `713.2 GB`
- Representative file paths:
  - `115.Liveness Detection을 위한 영상 데이터/01.데이터/1.Training/원천데이터(zip)/0991-1145.zip`
  - `115.Liveness Detection을 위한 영상 데이터/01.데이터/1.Training/원천데이터(zip)/1146-1313.zip`
  - `115.Liveness Detection을 위한 영상 데이터/01.데이터/1.Training/원천데이터(zip)/1314-1474.zip`
  - `115.Liveness Detection을 위한 영상 데이터/01.데이터/1.Training/원천데이터(zip)/1475-1624.zip`
  - `115.Liveness Detection을 위한 영상 데이터/01.데이터/1.Training/원천데이터(zip)/1625-1782.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 33개이며 알려진 총 용량은 713.2 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
{
  "dataset_key": 161,
  "title": "Liveness Detection을 위한 영상",
  "tags": [
    "computer-vision",
    "video",
    "영상",
