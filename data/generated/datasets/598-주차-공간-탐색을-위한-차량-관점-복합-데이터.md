...
## Basic Info
- Dataset Key: `598`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 598`
- Collected At: `2026-03-17T08:40:52.744116+00:00`
...
## File Overview
- File count: `28`
- Total known size: `289.3 GB`
- Representative file paths:
  - `088.주차 공간 탐색을 위한 차량 관점 복합 데이터/01.데이터/1.Training/원천데이터/TS8_실외_중소형주차장.zip`
  - `088.주차 공간 탐색을 위한 차량 관점 복합 데이터/01.데이터/1.Training/원천데이터/TS1_실내_대형주차장.zip`
  - `088.주차 공간 탐색을 위한 차량 관점 복합 데이터/01.데이터/1.Training/원천데이터/TS2_실내_중소형주차장.zip`
  - `088.주차 공간 탐색을 위한 차량 관점 복합 데이터/01.데이터/1.Training/원천데이터/TS3_실내_중소형주차장.zip`
  - `088.주차 공간 탐색을 위한 차량 관점 복합 데이터/01.데이터/1.Training/원천데이터/TS4_실외_노상.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 28개이며 알려진 총 용량은 289.3 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
{
  "dataset_key": 598,
  "title": "주차 공간 탐색을 위한 차량 관점 복합 데이터",
  "tags": [
    "공간",
    "관점",
    "복합",
