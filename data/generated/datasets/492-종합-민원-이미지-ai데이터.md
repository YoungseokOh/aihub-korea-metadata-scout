...
## Basic Info
- Dataset Key: `492`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 492`
- Collected At: `2026-03-17T08:40:43.263244+00:00`
...
## File Overview
- File count: `20`
- Total known size: `27.7 GB`
- Representative file paths:
  - `138.종합 민원 이미지 AI데이터/01.데이터/1.Training/라벨링데이터/TL1.zip`
  - `138.종합 민원 이미지 AI데이터/01.데이터/1.Training/라벨링데이터/TL2.zip`
  - `138.종합 민원 이미지 AI데이터/01.데이터/1.Training/라벨링데이터/TL3.zip`
  - `138.종합 민원 이미지 AI데이터/01.데이터/1.Training/라벨링데이터/TL4.zip`
  - `138.종합 민원 이미지 AI데이터/01.데이터/1.Training/라벨링데이터/TL5.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `legal/compliance` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 20개이며 알려진 총 용량은 27.7 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
