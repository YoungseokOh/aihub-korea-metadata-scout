...
## Basic Info
- Dataset Key: `177`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 177`
- Collected At: `2026-03-17T08:40:35.172767+00:00`
...
## File Overview
- File count: `42`
- Total known size: `972.8 GB`
- Representative file paths:
  - `104.항공 활주로 내 이상물체 감지를 위한 객체 데이터/01.데이터/1.Training/원천데이터(zip)/04.스튜디오(연출_및_구매)_6.zip`
  - `104.항공 활주로 내 이상물체 감지를 위한 객체 데이터/01.데이터/1.Training/원천데이터(zip)/04.스튜디오(연출_및_구매)_7.zip`
  - `104.항공 활주로 내 이상물체 감지를 위한 객체 데이터/01.데이터/1.Training/원천데이터(zip)/04.스튜디오(연출_및_구매)_8.zip`
  - `104.항공 활주로 내 이상물체 감지를 위한 객체 데이터/01.데이터/1.Training/원천데이터(zip)/01.한서대_비행교육장_6.zip`
  - `104.항공 활주로 내 이상물체 감지를 위한 객체 데이터/01.데이터/1.Training/원천데이터(zip)/01.한서대_비행교육장_7.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `education` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 42개이며 알려진 총 용량은 972.8 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 42,
  "total_size_bytes": 1044573585408,
  "human_size": "972.8 GB",
  "sample_file_paths": [
    "104.항공 활주로 내 이상물체 감지를 위한 객체 데이터/01.데이터/1.Training/원천데이터(zip)/04.스튜디오(연출_및_구매)_6.zip",
    "104.항공 활주로 내 이상물체 감지를 위한 객체 데이터/01.데이터/1.Training/원천데이터(zip)/04.스튜디오(연출_및_구매)_7.zip",
