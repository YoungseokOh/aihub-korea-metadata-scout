...
## Basic Info
- Dataset Key: `137`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 137`
- Collected At: `2026-03-17T08:40:30.904694+00:00`
...
## File Overview
- File count: `49`
- Total known size: `465.9 GB`
- Representative file paths:
  - `084.산업 폐기물 이미지/01.데이터/1.Training/원천데이터/유리도자기류/유리도자기류.zip`
  - `084.산업 폐기물 이미지/01.데이터/1.Training/원천데이터/타이어/타이어1.zip`
  - `084.산업 폐기물 이미지/01.데이터/1.Training/원천데이터/타이어/타이어2.zip`
  - `084.산업 폐기물 이미지/01.데이터/1.Training/원천데이터/폐목재류/폐목재류1.zip`
  - `084.산업 폐기물 이미지/01.데이터/1.Training/원천데이터/폐목재류/폐목재류2.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 49개이며 알려진 총 용량은 465.9 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
