...
## Basic Info
- Dataset Key: `59`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 59`
- Collected At: `2026-03-17T08:40:21.470075+00:00`
...
## File Overview
- File count: `100`
- Total known size: `108.7 GB`
- Representative file paths:
  - `123.반려동물 구분을 위한 동물 영상 데이터셋(종류 구분, 행동 구분)/01.데이터/1.Training/라벨링데이터_09/CAT/ARCH.zip`
  - `123.반려동물 구분을 위한 동물 영상 데이터셋(종류 구분, 행동 구분)/01.데이터/1.Training/라벨링데이터_09/CAT/ARMSTRETCH.zip`
  - `123.반려동물 구분을 위한 동물 영상 데이터셋(종류 구분, 행동 구분)/01.데이터/1.Training/라벨링데이터_09/CAT/FOOTPUSH.zip`
  - `123.반려동물 구분을 위한 동물 영상 데이터셋(종류 구분, 행동 구분)/01.데이터/1.Training/라벨링데이터_09/CAT/GETDOWN.zip`
  - `123.반려동물 구분을 위한 동물 영상 데이터셋(종류 구분, 행동 구분)/01.데이터/1.Training/라벨링데이터_09/CAT/GROOMING.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 100개이며 알려진 총 용량은 108.7 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
