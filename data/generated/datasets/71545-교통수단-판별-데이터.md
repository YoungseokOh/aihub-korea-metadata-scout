...
## Basic Info
- Dataset Key: `71545`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71545`
- Collected At: `2026-03-17T08:41:22.895713+00:00`
...
## File Overview
- File count: `71`
- Total known size: `103.9 GB`
- Representative file paths:
  - `186.교통수단판별 데이터/01-1.정식개방데이터/Other/01.연계데이터_001.대중교통카드.zip`
  - `186.교통수단판별 데이터/01-1.정식개방데이터/Other/01.연계데이터_002.PM데이터.zip`
  - `186.교통수단판별 데이터/01-1.정식개방데이터/Other/01.연계데이터_003.차량이동궤적_1.zip`
  - `186.교통수단판별 데이터/01-1.정식개방데이터/Other/01.연계데이터_003.차량이동궤적_2.zip`
  - `186.교통수단판별 데이터/01-1.정식개방데이터/Other/01.연계데이터_004.대중교통궤적.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 71개이며 알려진 총 용량은 103.9 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
