...
## Basic Info
- Dataset Key: `653`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 653`
- Collected At: `2026-03-17T08:40:56.341662+00:00`
...
## File Overview
- File count: `158`
- Total known size: `15.9 GB`
- Representative file paths:
  - `029.대규모 구매도서 기반 한국어 말뭉치 데이터/01.데이터/1.Training/원천데이터/TS_600.zip`
  - `029.대규모 구매도서 기반 한국어 말뭉치 데이터/01.데이터/1.Training/원천데이터/TS_320.zip`
  - `029.대규모 구매도서 기반 한국어 말뭉치 데이터/01.데이터/1.Training/원천데이터/TS_360.zip`
  - `029.대규모 구매도서 기반 한국어 말뭉치 데이터/01.데이터/1.Training/원천데이터/TS_370.zip`
  - `029.대규모 구매도서 기반 한국어 말뭉치 데이터/01.데이터/1.Training/원천데이터/TS_380.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `nlp/knowledge` 성격의 `text` 자료로 보입니다. 확인 가능한 파일은 158개이며 알려진 총 용량은 15.9 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
