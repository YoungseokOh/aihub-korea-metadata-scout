...
## Basic Info
- Dataset Key: `131`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 131`
- Collected At: `2026-03-17T08:40:30.324002+00:00`
...
## File Overview
- File count: `22`
- Total known size: `399.6 GB`
- Representative file paths:
  - `004.한국인 외래어 발화/01.데이터/1.Training/2.원천데이터_0913_add/2.음성수집도구/2.음성수집도구_7_한국인외래어발화_TRAINING.zip`
  - `004.한국인 외래어 발화/01.데이터/1.Training/2.원천데이터_0913_add/2.음성수집도구/2.음성수집도구_8_한국인외래어발화_TRAINING.zip`
  - `004.한국인 외래어 발화/01.데이터/1.Training/2.원천데이터_0913_add/2.음성수집도구/2.음성수집도구_1_한국인외래어발화_TRAINING.zip`
  - `004.한국인 외래어 발화/01.데이터/1.Training/2.원천데이터_0913_add/2.음성수집도구/2.음성수집도구_2_한국인외래어발화_TRAINING.zip`
  - `004.한국인 외래어 발화/01.데이터/1.Training/2.원천데이터_0913_add/2.음성수집도구/2.음성수집도구_3_한국인외래어발화_TRAINING.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `speech/dialogue` 성격의 `audio` 자료로 보입니다. 확인 가능한 파일은 22개이며 알려진 총 용량은 399.6 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
