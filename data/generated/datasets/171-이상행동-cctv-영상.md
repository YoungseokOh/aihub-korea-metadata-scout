...
## Basic Info
- Dataset Key: `171`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 171`
- Collected At: `2026-03-17T08:40:34.519972+00:00`
...
## File Overview
- File count: `189`
- Total known size: `4.84 TB`
- Representative file paths:
  - `19.이상행동CCTV/01.폭행(assault)/실외/outsidedoor_09.zip`
  - `19.이상행동CCTV/01.폭행(assault)/실외/outsidedoor_01.zip`
  - `19.이상행동CCTV/01.폭행(assault)/실외/outsidedoor_02.zip`
  - `19.이상행동CCTV/01.폭행(assault)/실외/outsidedoor_03.zip`
  - `19.이상행동CCTV/01.폭행(assault)/실외/outsidedoor_04.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 189개이며 알려진 총 용량은 4.84 TB입니다. 압축 파일 조각이 많아 보여 파일 정리와 전처리 부담이 커질 수 있습니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 189,
  "total_size_bytes": 5325759447040,
  "human_size": "4.84 TB",
  "sample_file_paths": [
    "19.이상행동CCTV/01.폭행(assault)/실외/outsidedoor_09.zip",
    "19.이상행동CCTV/01.폭행(assault)/실외/outsidedoor_01.zip",
