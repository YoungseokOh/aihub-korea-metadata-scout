...
## Basic Info
- Dataset Key: `554`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 554`
- Collected At: `2026-03-17T08:40:47.755601+00:00`
...
## File Overview
- File count: `16`
- Total known size: `79.4 GB`
- Representative file paths:
  - `091.차량 외관 영상 데이터/01.데이터/2.Validation/원천데이터/VS1.zip`
  - `091.차량 외관 영상 데이터/01.데이터/2.Validation/원천데이터/VS2.zip`
  - `091.차량 외관 영상 데이터/01.데이터/2.Validation/원천데이터/VS3.zip`
  - `091.차량 외관 영상 데이터/01.데이터/2.Validation/원천데이터/VS4.zip`
  - `091.차량 외관 영상 데이터/01.데이터/2.Validation/라벨링데이터/VL1.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 16개이며 알려진 총 용량은 79.4 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
