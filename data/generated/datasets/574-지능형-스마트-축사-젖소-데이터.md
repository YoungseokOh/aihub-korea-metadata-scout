...
## Basic Info
- Dataset Key: `574`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 574`
- Collected At: `2026-03-17T08:40:49.441769+00:00`
...
## File Overview
- File count: `226`
- Total known size: `5.09 TB`
- Representative file paths:
  - `168.스마트축사 통합 데이터(젖소)/01.데이터/1. Training/원천데이터_240412_add/젖소유방이미지_건유기.tar.gz`
  - `168.스마트축사 통합 데이터(젖소)/01.데이터/1. Training/원천데이터_240412_add/젖소유방이미지_비유중기.tar.gz`
  - `168.스마트축사 통합 데이터(젖소)/01.데이터/1. Training/원천데이터_240412_add/젖소유방이미지_비유초기.tar.gz`
  - `168.스마트축사 통합 데이터(젖소)/01.데이터/1. Training/원천데이터_240412_add/젖소유방이미지_비유후기.tar.gz`
  - `168.스마트축사 통합 데이터(젖소)/01.데이터/1. Training/원천데이터_240412_add/젖소이미지_경산우_BCS.tar.gz`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `speech/dialogue` 성격의 `audio` 자료로 보입니다. 확인 가능한 파일은 226개이며 알려진 총 용량은 5.09 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 226,
  "total_size_bytes": 5599951663104,
  "human_size": "5.09 TB",
  "sample_file_paths": [
    "168.스마트축사 통합 데이터(젖소)/01.데이터/1. Training/원천데이터_240412_add/젖소유방이미지_건유기.tar.gz",
    "168.스마트축사 통합 데이터(젖소)/01.데이터/1. Training/원천데이터_240412_add/젖소유방이미지_비유중기.tar.gz",
