...
## Basic Info
- Dataset Key: `71601`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71601`
- Collected At: `2026-03-17T08:41:27.405073+00:00`
...
- Representative file paths:
  - `105.손·팔 협조에 의한 파지-조작 동작 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_000.zip`
  - `105.손·팔 협조에 의한 파지-조작 동작 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_000_rgb.zip`
  - `105.손·팔 협조에 의한 파지-조작 동작 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_001.zip`
  - `105.손·팔 협조에 의한 파지-조작 동작 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_001_rgb.zip`
  - `105.손·팔 협조에 의한 파지-조작 동작 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_002.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 546개이며 알려진 총 용량은 1.94 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "modality_guess": null,
  "file_count": 546,
  "total_size_bytes": 2130699815936,
  "human_size": "1.94 TB",
  "sample_file_paths": [
    "105.손·팔 협조에 의한 파지-조작 동작 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_000.zip",
    "105.손·팔 협조에 의한 파지-조작 동작 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_000_rgb.zip",
