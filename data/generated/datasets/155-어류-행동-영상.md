...
## Basic Info
- Dataset Key: `155`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 155`
- Collected At: `2026-03-17T08:40:32.654007+00:00`
...
## File Overview
- File count: `22`
- Total known size: `199.7 GB`
- Representative file paths:
  - `077.어류 행동 영상/01.데이터/1.Training/라벨링데이터/bp_part1.json`
  - `077.어류 행동 영상/01.데이터/1.Training/라벨링데이터/bp_part2.json`
  - `077.어류 행동 영상/01.데이터/1.Training/라벨링데이터/kr_part1.json`
  - `077.어류 행동 영상/01.데이터/1.Training/라벨링데이터/kr_part2.json`
  - `077.어류 행동 영상/01.데이터/1.Training/라벨링데이터/of_part1.json`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 22개이며 알려진 총 용량은 199.7 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
