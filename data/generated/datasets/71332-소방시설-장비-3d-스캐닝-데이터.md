...
## Basic Info
- Dataset Key: `71332`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71332`
- Collected At: `2026-03-17T08:41:02.577968+00:00`
...
## File Overview
- File count: `2`
- Total known size: `80.0 GB`
- Representative file paths:
  - `057.소방시설·장비 3D 스캐닝 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.zip`
  - `057.소방시설·장비 3D 스캐닝 데이터/01-1.정식개방데이터/Training/02.라벨링데이터/TL.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `3d/lidar` 자료로 보입니다. 확인 가능한 파일은 2개이며 알려진 총 용량은 80.0 GB입니다. 라벨링 데이터와 원천 데이터가 모두 보여서 지도학습 준비도가 상대적으로 높아 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
