...
## Basic Info
- Dataset Key: `130`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 130`
- Collected At: `2026-03-17T08:40:30.234706+00:00`
...
## File Overview
- File count: `78`
- Total known size: `323.2 GB`
- Representative file paths:
  - `009.한국인대화음성/01.데이터/1.Training(new)/1.라벨링데이터/4.생활/life_10.tar.gz`
  - `009.한국인대화음성/01.데이터/1.Training(new)/1.라벨링데이터/4.생활/life_01.tar.gz`
  - `009.한국인대화음성/01.데이터/1.Training(new)/1.라벨링데이터/4.생활/life_02.tar.gz`
  - `009.한국인대화음성/01.데이터/1.Training(new)/1.라벨링데이터/4.생활/life_03.tar.gz`
  - `009.한국인대화음성/01.데이터/1.Training(new)/1.라벨링데이터/4.생활/life_04.tar.gz`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `speech/dialogue` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 78개이며 알려진 총 용량은 323.2 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
