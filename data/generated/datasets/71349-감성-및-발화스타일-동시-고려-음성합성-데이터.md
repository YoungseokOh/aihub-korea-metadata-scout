...
## Basic Info
- Dataset Key: `71349`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71349`
- Collected At: `2026-03-17T08:41:03.692240+00:00`
...
## File Overview
- File count: `1412`
- Total known size: `217.2 GB`
- Representative file paths:
  - `133.감성 및 발화 스타일 동시 고려 음성합성 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_구연체_001.zip`
  - `133.감성 및 발화 스타일 동시 고려 음성합성 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_구연체_005.zip`
  - `133.감성 및 발화 스타일 동시 고려 음성합성 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_구연체_017.zip`
  - `133.감성 및 발화 스타일 동시 고려 음성합성 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_구연체_021.zip`
  - `133.감성 및 발화 스타일 동시 고려 음성합성 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_구연체_022.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `speech/dialogue` 성격의 `audio` 자료로 보입니다. 확인 가능한 파일은 1412개이며 알려진 총 용량은 217.2 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
{
  "dataset_key": 71349,
  "title": "감성 및 발화스타일 동시 고려 음성합성 데이터",
  "tags": [
    "speech/dialogue",
    "audio",
    "대화",
