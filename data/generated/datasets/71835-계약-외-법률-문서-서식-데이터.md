...
## Basic Info
- Dataset Key: `71835`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71835`
- Collected At: `2026-03-17T08:41:43.206117+00:00`
...
## File Overview
- File count: `44`
- Total known size: `43.2 MB`
- Representative file paths:
  - `06.계약 외 법률 문서 서식 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_내용증명_기타_기타.zip`
  - `06.계약 외 법률 문서 서식 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_내용증명_기타_내용증명.zip`
  - `06.계약 외 법률 문서 서식 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_민원행정_동의_위임관련문서.zip`
  - `06.계약 외 법률 문서 서식 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_민원행정_요청_신고관련문서.zip`
  - `06.계약 외 법률 문서 서식 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_불복신청_불복이유서.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `ocr/document` 성격의 `text` 자료로 보입니다. 확인 가능한 파일은 44개이며 알려진 총 용량은 43.2 MB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
