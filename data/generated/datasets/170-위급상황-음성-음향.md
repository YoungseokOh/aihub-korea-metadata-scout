...
## Basic Info
- Dataset Key: `170`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 170`
- Collected At: `2026-03-17T08:40:34.419783+00:00`
...
## File Overview
- File count: `70`
- Total known size: `507.7 GB`
- Representative file paths:
  - `088.위급상황 음성-음향 데이터/01.데이터/1.Training/라벨링데이터_수정/1.강제추행(성범죄).zip`
  - `088.위급상황 음성-음향 데이터/01.데이터/1.Training/라벨링데이터_수정/2.강도범죄.zip`
  - `088.위급상황 음성-음향 데이터/01.데이터/1.Training/라벨링데이터_수정/9.가스사고.zip`
  - `088.위급상황 음성-음향 데이터/01.데이터/1.Training/라벨링데이터_수정/10.낙상.zip`
  - `088.위급상황 음성-음향 데이터/01.데이터/1.Training/라벨링데이터_수정/11.붕괴사고.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `speech/dialogue` 성격의 `audio` 자료로 보입니다. 확인 가능한 파일은 70개이며 알려진 총 용량은 507.7 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
