...
## Basic Info
- Dataset Key: `635`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 635`
- Collected At: `2026-03-17T08:40:55.042957+00:00`
...
## File Overview
- File count: `41`
- Total known size: `967.5 GB`
- Representative file paths:
  - `169.고품질 과수 작물 통합 데이터(이미지)/01.데이터/1.Training/원천데이터/TS4.초분광_키위8.zip`
  - `169.고품질 과수 작물 통합 데이터(이미지)/01.데이터/1.Training/원천데이터/TS5.기상환경데이터.zip`
  - `169.고품질 과수 작물 통합 데이터(이미지)/01.데이터/1.Training/원천데이터/TS1.감귤.zip`
  - `169.고품질 과수 작물 통합 데이터(이미지)/01.데이터/1.Training/원천데이터/TS2.키위.zip`
  - `169.고품질 과수 작물 통합 데이터(이미지)/01.데이터/1.Training/원천데이터/TS3.초분광_감귤1.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 41개이며 알려진 총 용량은 967.5 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 41,
  "total_size_bytes": 1038819000320,
  "human_size": "967.5 GB",
  "sample_file_paths": [
    "169.고품질 과수 작물 통합 데이터(이미지)/01.데이터/1.Training/원천데이터/TS4.초분광_키위8.zip",
    "169.고품질 과수 작물 통합 데이터(이미지)/01.데이터/1.Training/원천데이터/TS5.기상환경데이터.zip",
