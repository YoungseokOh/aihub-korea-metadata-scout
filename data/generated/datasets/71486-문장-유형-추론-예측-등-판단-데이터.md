# 문장 유형(추론, 예측 등) 판단 데이터

## Basic Info
- Dataset Key: `71486`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71486`
- Collected At: `2026-03-17T08:41:16.483762+00:00`
...
## File Overview
- File count: `68`
- Total known size: `27.2 MB`
- Representative file paths:
  - `159.문장 유형(추론, 예측 등) 판단 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_뉴스_IT과학.zip`
  - `159.문장 유형(추론, 예측 등) 판단 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_뉴스_금융.zip`
  - `159.문장 유형(추론, 예측 등) 판단 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_뉴스_문화.zip`
  - `159.문장 유형(추론, 예측 등) 판단 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_뉴스_사회.zip`
  - `159.문장 유형(추론, 예측 등) 판단 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_뉴스_생활건강.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 68개이며 알려진 총 용량은 27.2 MB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
