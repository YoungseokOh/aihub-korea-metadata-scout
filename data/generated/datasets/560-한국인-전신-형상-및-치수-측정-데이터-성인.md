...
## Basic Info
- Dataset Key: `560`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 560`
- Collected At: `2026-03-17T08:40:48.285641+00:00`
...
## File Overview
- File count: `29`
- Total known size: `436.2 GB`
- Representative file paths:
  - `151.한국인 전신 형상 및 치수 측정 데이터/01.데이터/1.Training/라벨링데이터_230524add/PLY.zip`
  - `151.한국인 전신 형상 및 치수 측정 데이터/01.데이터/1.Training/원천데이터_230524add/신체정보데이터.zip`
  - `151.한국인 전신 형상 및 치수 측정 데이터/01.데이터/1.Training/라벨링데이터/TL_M409toM505.zip`
  - `151.한국인 전신 형상 및 치수 측정 데이터/01.데이터/1.Training/라벨링데이터/TL_F009toF108.zip`
  - `151.한국인 전신 형상 및 치수 측정 데이터/01.데이터/1.Training/라벨링데이터/TL_F109toF208.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `3d/lidar` 자료로 보입니다. 확인 가능한 파일은 29개이며 알려진 총 용량은 436.2 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
{
  "dataset_key": 560,
  "title": "한국인 전신 형상 및 치수 측정 데이터(성인)",
  "tags": [
    "3d/lidar",
    "3d",
    "성인",
