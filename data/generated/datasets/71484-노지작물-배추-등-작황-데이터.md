...
## Basic Info
- Dataset Key: `71484`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71484`
- Collected At: `2026-03-17T08:41:16.277591+00:00`
...
## File Overview
- File count: `44`
- Total known size: `369.9 GB`
- Representative file paths:
  - `107.노지작물(배추 등) 작황 데이터/01-1.정식개방데이터/Training/02.라벨링데이터/TL_3.기타작물_4.제주도.zip`
  - `107.노지작물(배추 등) 작황 데이터/01-1.정식개방데이터/Training/02.라벨링데이터/TL_1.배추_1.강원도.zip`
  - `107.노지작물(배추 등) 작황 데이터/01-1.정식개방데이터/Training/02.라벨링데이터/TL_1.배추_2.전라도.zip`
  - `107.노지작물(배추 등) 작황 데이터/01-1.정식개방데이터/Training/02.라벨링데이터/TL_1.배추_3.충청도.zip`
  - `107.노지작물(배추 등) 작황 데이터/01-1.정식개방데이터/Training/02.라벨링데이터/TL_1.배추_4.제주도.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 44개이며 알려진 총 용량은 369.9 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
