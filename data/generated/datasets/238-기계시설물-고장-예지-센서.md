...
## Basic Info
- Dataset Key: `238`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 238`
- Collected At: `2026-03-17T08:40:39.489323+00:00`
...
## File Overview
- File count: `4`
- Total known size: `112.0 GB`
- Representative file paths:
  - `125.기계시설물 고장 예지 센서/01.데이터/1.Training/current.zip`
  - `125.기계시설물 고장 예지 센서/01.데이터/1.Training/vibration.zip`
  - `125.기계시설물 고장 예지 센서/01.데이터/2. Validation/current.zip`
  - `125.기계시설물 고장 예지 센서/01.데이터/2. Validation/vibration.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 4개이며 알려진 총 용량은 112.0 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
