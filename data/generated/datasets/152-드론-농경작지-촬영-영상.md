...
## Basic Info
- Dataset Key: `152`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 152`
- Collected At: `2026-03-17T08:40:32.325745+00:00`
...
## File Overview
- File count: `591`
- Total known size: `5.93 TB`
- Representative file paths:
  - `067.드론 농경작지 촬영 영상/01.데이터/1.Training/원천데이터/Zone-L-009.zip`
  - `067.드론 농경작지 촬영 영상/01.데이터/1.Training/원천데이터/Zone-L-010.zip`
  - `067.드론 농경작지 촬영 영상/01.데이터/1.Training/원천데이터/Zone-L-011.zip`
  - `067.드론 농경작지 촬영 영상/01.데이터/1.Training/원천데이터/Zone-L-012.zip`
  - `067.드론 농경작지 촬영 영상/01.데이터/1.Training/원천데이터/Zone-L-013.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 591개이며 알려진 총 용량은 5.93 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 591,
  "total_size_bytes": 6522983677952,
  "human_size": "5.93 TB",
  "sample_file_paths": [
    "067.드론 농경작지 촬영 영상/01.데이터/1.Training/원천데이터/Zone-L-009.zip",
    "067.드론 농경작지 촬영 영상/01.데이터/1.Training/원천데이터/Zone-L-010.zip",
