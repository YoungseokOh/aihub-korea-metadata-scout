...
## Basic Info
- Dataset Key: `493`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 493`
- Collected At: `2026-03-17T08:40:43.353978+00:00`
...
## File Overview
- File count: `88`
- Total known size: `2.98 TB`
- Representative file paths:
  - `047.대용량 3D 객체 데이터/01.데이터/1.Training/원천데이터/TS20.zip`
  - `047.대용량 3D 객체 데이터/01.데이터/1.Training/원천데이터/TS21.zip`
  - `047.대용량 3D 객체 데이터/01.데이터/1.Training/원천데이터/TS22.zip`
  - `047.대용량 3D 객체 데이터/01.데이터/1.Training/원천데이터/TS23.zip`
  - `047.대용량 3D 객체 데이터/01.데이터/1.Training/원천데이터/TS24.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `3d/lidar` 자료로 보입니다. 확인 가능한 파일은 88개이며 알려진 총 용량은 2.98 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 88,
  "total_size_bytes": 3273769615360,
  "human_size": "2.98 TB",
  "sample_file_paths": [
    "047.대용량 3D 객체 데이터/01.데이터/1.Training/원천데이터/TS20.zip",
    "047.대용량 3D 객체 데이터/01.데이터/1.Training/원천데이터/TS21.zip",
