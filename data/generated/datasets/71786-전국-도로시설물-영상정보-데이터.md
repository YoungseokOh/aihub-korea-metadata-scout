...
## Basic Info
- Dataset Key: `71786`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71786`
- Collected At: `2026-03-17T08:41:40.148158+00:00`
...
## File Overview
- File count: `37`
- Total known size: `1.68 TB`
- Representative file paths:
  - `059.전국 도로시설물 영상정보 데이터/3.개방데이터/1.데이터/Other/Other_1.zip`
  - `059.전국 도로시설물 영상정보 데이터/3.개방데이터/1.데이터/Other/Other_10.zip`
  - `059.전국 도로시설물 영상정보 데이터/3.개방데이터/1.데이터/Other/Other_11.zip`
  - `059.전국 도로시설물 영상정보 데이터/3.개방데이터/1.데이터/Other/Other_12.zip`
  - `059.전국 도로시설물 영상정보 데이터/3.개방데이터/1.데이터/Other/Other_13.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 37개이며 알려진 총 용량은 1.68 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 37,
  "total_size_bytes": 1844883668992,
  "human_size": "1.68 TB",
  "sample_file_paths": [
    "059.전국 도로시설물 영상정보 데이터/3.개방데이터/1.데이터/Other/Other_1.zip",
    "059.전국 도로시설물 영상정보 데이터/3.개방데이터/1.데이터/Other/Other_10.zip",
