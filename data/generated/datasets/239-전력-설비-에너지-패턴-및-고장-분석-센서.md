...
## Basic Info
- Dataset Key: `239`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 239`
- Collected At: `2026-03-17T08:40:39.573664+00:00`
...
## File Overview
- File count: `40`
- Total known size: `19.5 GB`
- Representative file paths:
  - `149.전력 설비 에너지 품질/01.데이터/1.Training/라벨링데이터/1.펌프_일반모터.zip`
  - `149.전력 설비 에너지 품질/01.데이터/1.Training/라벨링데이터/2.공조설비.zip`
  - `149.전력 설비 에너지 품질/01.데이터/1.Training/라벨링데이터/3.공기압축기.zip`
  - `149.전력 설비 에너지 품질/01.데이터/1.Training/라벨링데이터/4.공작기계.zip`
  - `149.전력 설비 에너지 품질/01.데이터/1.Training/라벨링데이터/5.보일러.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 40개이며 알려진 총 용량은 19.5 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
