...
## Basic Info
- Dataset Key: `71777`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71777`
- Collected At: `2026-03-17T08:41:39.607610+00:00`
...
## File Overview
- File count: `54`
- Total known size: `4.91 TB`
- Representative file paths:
  - `128.김치 생산공정별 비파괴 품질검사를 위한 데이터/3.개방데이터/1.데이터/Other/Other.zip`
  - `128.김치 생산공정별 비파괴 품질검사를 위한 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS.z01`
  - `128.김치 생산공정별 비파괴 품질검사를 위한 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS.z02`
  - `128.김치 생산공정별 비파괴 품질검사를 위한 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS.z03`
  - `128.김치 생산공정별 비파괴 품질검사를 위한 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS.z04`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 54개이며 알려진 총 용량은 4.91 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
{
  "dataset_key": 71777,
  "title": "김치 생산공정별 비파괴 품질검사를 위한 데이터셋 구축",
  "tags": [
    "구축",
    "김치",
    "데이터셋",
  ...
  "modality_guess": null,
  "file_count": 54,
  "total_size_bytes": 5401050349568,
  "human_size": "4.91 TB",
  "sample_file_paths": [
    "128.김치 생산공정별 비파괴 품질검사를 위한 데이터/3.개방데이터/1.데이터/Other/Other.zip",
    "128.김치 생산공정별 비파괴 품질검사를 위한 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS.z01",
