...
## Basic Info
- Dataset Key: `522`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 522`
- Collected At: `2026-03-17T08:40:45.209224+00:00`
...
## File Overview
- File count: `20`
- Total known size: `198.2 GB`
- Representative file paths:
  - `094.교차로 신호 체계, 보행자, 차량 이동 복합 데이터/01.데이터/1.Training/라벨링데이터/TL_1.통과차량.zip`
  - `094.교차로 신호 체계, 보행자, 차량 이동 복합 데이터/01.데이터/1.Training/라벨링데이터/TL_2.보행량.zip`
  - `094.교차로 신호 체계, 보행자, 차량 이동 복합 데이터/01.데이터/1.Training/라벨링데이터/TL_3.도로차량.zip`
  - `094.교차로 신호 체계, 보행자, 차량 이동 복합 데이터/01.데이터/1.Training/라벨링데이터/TL_4.바운딩박스.zip`
  - `094.교차로 신호 체계, 보행자, 차량 이동 복합 데이터/01.데이터/1.Training/라벨링데이터/TL_5.큐보이드.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 20개이며 알려진 총 용량은 198.2 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
{
  "dataset_key": 522,
  "title": "교차로 신호 체계, 보행자, 차량 이동 복합 데이터",
  "tags": [
    "교차로",
    "보행자",
    "복합",
