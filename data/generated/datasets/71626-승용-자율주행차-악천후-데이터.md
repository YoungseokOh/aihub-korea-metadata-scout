...
## Basic Info
- Dataset Key: `71626`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71626`
- Collected At: `2026-03-17T08:41:29.168780+00:00`
...
## File Overview
- File count: `7`
- Total known size: `277.3 GB`
- Representative file paths:
  - `092.승용 자율주행차 악천후 데이터/01-1.정식개방데이터/Other/Other.zip`
  - `092.승용 자율주행차 악천후 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z01`
  - `092.승용 자율주행차 악천후 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z02`
  - `092.승용 자율주행차 악천후 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.zip`
  - `092.승용 자율주행차 악천후 데이터/01-1.정식개방데이터/Training/02.라벨링데이터/TL.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 7개이며 알려진 총 용량은 277.3 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
