...
## Basic Info
- Dataset Key: `475`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 475`
- Collected At: `2026-03-17T08:40:41.903731+00:00`
...
## File Overview
- File count: `28`
- Total known size: `850.3 GB`
- Representative file paths:
  - `176.공룡발자국 화석 이미지 데이터/01.데이터/1.Training/원천데이터/TS11_3D.zip`
  - `176.공룡발자국 화석 이미지 데이터/01.데이터/1.Training/원천데이터/TS01_CA.zip`
  - `176.공룡발자국 화석 이미지 데이터/01.데이터/1.Training/원천데이터/TS02_CA.zip`
  - `176.공룡발자국 화석 이미지 데이터/01.데이터/1.Training/원천데이터/TS03_CA.zip`
  - `176.공룡발자국 화석 이미지 데이터/01.데이터/1.Training/원천데이터/TS04_CA.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `3d/lidar` 자료로 보입니다. 확인 가능한 파일은 28개이며 알려진 총 용량은 850.3 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
