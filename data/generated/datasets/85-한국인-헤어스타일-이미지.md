...
## Basic Info
- Dataset Key: `85`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 85`
- Collected At: `2026-03-17T08:40:25.454640+00:00`
...
## File Overview
- File count: `39`
- Total known size: `678.8 GB`
- Representative file paths:
  - `132.한국인 헤어스타일 이미지/01.데이터/1.Training/2.원천데이터/mqset001.zip`
  - `132.한국인 헤어스타일 이미지/01.데이터/1.Training/2.원천데이터/rawset001.zip`
  - `132.한국인 헤어스타일 이미지/01.데이터/1.Training/2.원천데이터/rawset002.zip`
  - `132.한국인 헤어스타일 이미지/01.데이터/1.Training/2.원천데이터/rawset003.zip`
  - `132.한국인 헤어스타일 이미지/01.데이터/1.Training/2.원천데이터/rawset004.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 39개이며 알려진 총 용량은 678.8 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
