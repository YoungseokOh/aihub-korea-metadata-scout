...
## Basic Info
- Dataset Key: `157`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 157`
- Collected At: `2026-03-17T08:40:32.852289+00:00`
...
## File Overview
- File count: `145`
- Total known size: `1.76 TB`
- Representative file paths:
  - `070.주요 농작물 생육 이미지 데이터/01.데이터/1.Training/라벨링데이터/02.콩/품질_수분.zip`
  - `070.주요 농작물 생육 이미지 데이터/01.데이터/1.Training/라벨링데이터/02.콩/수확량.zip`
  - `070.주요 농작물 생육 이미지 데이터/01.데이터/1.Training/라벨링데이터/02.콩/품질_단백질.zip`
  - `070.주요 농작물 생육 이미지 데이터/01.데이터/1.Training/라벨링데이터/03.무/수확량.zip`
  - `070.주요 농작물 생육 이미지 데이터/01.데이터/1.Training/라벨링데이터/03.무/품질.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 145개이며 알려진 총 용량은 1.76 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 145,
  "total_size_bytes": 1938249111552,
  "human_size": "1.76 TB",
  "sample_file_paths": [
    "070.주요 농작물 생육 이미지 데이터/01.데이터/1.Training/라벨링데이터/02.콩/품질_수분.zip",
    "070.주요 농작물 생육 이미지 데이터/01.데이터/1.Training/라벨링데이터/02.콩/수확량.zip",
