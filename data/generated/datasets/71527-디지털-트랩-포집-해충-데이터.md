...
## Basic Info
- Dataset Key: `71527`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71527`
- Collected At: `2026-03-17T08:41:21.253382+00:00`
...
## File Overview
- File count: `39`
- Total known size: `666.8 GB`
- Representative file paths:
  - `199.디지털 트랩 포집 해충 데이터/01-1.정식개방데이터/Validation/02.라벨링데이터/VL_페로몬형_톱다리개미허리노린재.zip`
  - `199.디지털 트랩 포집 해충 데이터/01-1.정식개방데이터/Validation/02.라벨링데이터/VL_끈끈이형_갈색날개매미충.zip`
  - `199.디지털 트랩 포집 해충 데이터/01-1.정식개방데이터/Validation/02.라벨링데이터/VL_끈끈이형_미국선녀벌레.zip`
  - `199.디지털 트랩 포집 해충 데이터/01-1.정식개방데이터/Validation/02.라벨링데이터/VL_끈끈이형_썩덩나무노린재.zip`
  - `199.디지털 트랩 포집 해충 데이터/01-1.정식개방데이터/Validation/02.라벨링데이터/VL_끈끈이형_작은뿌리파리.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 39개이며 알려진 총 용량은 666.8 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
