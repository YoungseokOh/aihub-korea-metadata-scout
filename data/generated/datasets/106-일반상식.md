...
## Basic Info
- Dataset Key: `106`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 106`
- Collected At: `2026-03-17T08:40:27.437260+00:00`
...
## File Overview
- File count: `3`
- Total known size: `53.0 MB`
- Representative file paths:
  - `05.일반상식/nia_common_01_triples_일반상식(N-Triple 형식).zip`
  - `05.일반상식/nia_common_02_squad_질문, 답변, 제시문 말뭉치.zip`
  - `05.일반상식/nia_common_03_mutual_무형대용어, 상호참조복원.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `nlp/knowledge` 성격의 `text` 자료로 보입니다. 확인 가능한 파일은 3개이며 알려진 총 용량은 53.0 MB입니다. 파일 트리 정보가 제한적이어서 구조 판단은 보수적으로 해석해야 합니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
