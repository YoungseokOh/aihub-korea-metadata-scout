...
## Basic Info
- Dataset Key: `235`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 235`
- Collected At: `2026-03-17T08:40:39.162499+00:00`
...
## File Overview
- File count: `40`
- Total known size: `266.1 GB`
- Representative file paths:
  - `087.열화상 카메라 데이터(산업단지 등)/01.데이터/2.Validation/원천데이터/08. 사람/08. 사람.zip`
  - `087.열화상 카메라 데이터(산업단지 등)/01.데이터/2.Validation/원천데이터/09. 자동차/09. 자동차.zip`
  - `087.열화상 카메라 데이터(산업단지 등)/01.데이터/2.Validation/원천데이터/10. 배/10. 배.zip`
  - `087.열화상 카메라 데이터(산업단지 등)/01.데이터/2.Validation/원천데이터/01. 저장탱크/01. 저장탱크.zip`
  - `087.열화상 카메라 데이터(산업단지 등)/01.데이터/2.Validation/원천데이터/02. 이송밸브/02. 이송밸브.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 40개이며 알려진 총 용량은 266.1 GB입니다. 라벨링 데이터와 원천 데이터가 모두 보여서 지도학습 준비도가 상대적으로 높아 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
