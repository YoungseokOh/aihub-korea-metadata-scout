...
## Basic Info
- Dataset Key: `597`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 597`
- Collected At: `2026-03-17T08:40:52.622712+00:00`
...
## File Overview
- File count: `186`
- Total known size: `3.78 TB`
- Representative file paths:
  - `095.교통사고 영상 데이터/01.데이터/1.Training/라벨링데이터_231108_add/TL_차대보행자_영상_육교및지하도부근.zip`
  - `095.교통사고 영상 데이터/01.데이터/1.Training/라벨링데이터_231108_add/TL_차대보행자_영상_횡단보도(신호등없음).zip`
  - `095.교통사고 영상 데이터/01.데이터/1.Training/라벨링데이터_231108_add/TL_차대보행자_영상_횡단보도(신호등없음)부근.zip`
  - `095.교통사고 영상 데이터/01.데이터/1.Training/라벨링데이터_231108_add/TL_차대보행자_영상_횡단보도(신호등있음).zip`
  - `095.교통사고 영상 데이터/01.데이터/1.Training/라벨링데이터_231108_add/TL_차대보행자_영상_횡단보도(신호등있음)부근.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 186개이며 알려진 총 용량은 3.78 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 186,
  "total_size_bytes": 4158714628096,
  "human_size": "3.78 TB",
  "sample_file_paths": [
    "095.교통사고 영상 데이터/01.데이터/1.Training/라벨링데이터_231108_add/TL_차대보행자_영상_육교및지하도부근.zip",
    "095.교통사고 영상 데이터/01.데이터/1.Training/라벨링데이터_231108_add/TL_차대보행자_영상_횡단보도(신호등없음).zip",
