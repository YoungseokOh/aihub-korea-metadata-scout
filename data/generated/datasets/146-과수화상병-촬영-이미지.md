...
## Basic Info
- Dataset Key: `146`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 146`
- Collected At: `2026-03-17T08:40:31.701286+00:00`
...
## File Overview
- File count: `37`
- Total known size: `480.6 GB`
- Representative file paths:
  - `074.과수화상병 촬영 이미지/01.데이터/1.Training/원천데이터/02.사과/9.증강_(4).zip`
  - `074.과수화상병 촬영 이미지/01.데이터/1.Training/원천데이터/02.사과/9.증강_(5).zip`
  - `074.과수화상병 촬영 이미지/01.데이터/1.Training/원천데이터/02.사과/9.증강_(6).zip`
  - `074.과수화상병 촬영 이미지/01.데이터/1.Training/원천데이터/02.사과/9.증강_(7).zip`
  - `074.과수화상병 촬영 이미지/01.데이터/1.Training/원천데이터/02.사과/9.증강_(8).zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 37개이며 알려진 총 용량은 480.6 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
