...
## Basic Info
- Dataset Key: `154`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 154`
- Collected At: `2026-03-17T08:40:32.557425+00:00`
...
## File Overview
- File count: `8`
- Total known size: `89.1 GB`
- Representative file paths:
  - `078.어류 개체 촬영 영상/01.데이터/1.Training/라벨링데이터/gbt_fish_dtset.zip`
  - `078.어류 개체 촬영 영상/01.데이터/1.Training/원천데이터/dtset1.zip`
  - `078.어류 개체 촬영 영상/01.데이터/1.Training/원천데이터/dtset2.zip`
  - `078.어류 개체 촬영 영상/01.데이터/1.Training/원천데이터/dtset3.zip`
  - `078.어류 개체 촬영 영상/01.데이터/1.Training/원천데이터/dtset4.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 8개이며 알려진 총 용량은 89.1 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
