...
## Basic Info
- Dataset Key: `588`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 588`
- Collected At: `2026-03-17T08:40:50.702102+00:00`
...
## File Overview
- File count: `8`
- Total known size: `57.5 GB`
- Representative file paths:
  - `128.산림 수종 이미지 데이터(전라)/01.데이터/1.Training/라벨링데이터/TL1_Coarse.zip`
  - `128.산림 수종 이미지 데이터(전라)/01.데이터/1.Training/라벨링데이터/TL2_Fine.zip`
  - `128.산림 수종 이미지 데이터(전라)/01.데이터/1.Training/원천데이터/TS1_Coarse.zip`
  - `128.산림 수종 이미지 데이터(전라)/01.데이터/1.Training/원천데이터/TS2_Fine.zip`
  - `128.산림 수종 이미지 데이터(전라)/01.데이터/2.Validation/라벨링데이터/VL1_Coarse.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 8개이며 알려진 총 용량은 57.5 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
