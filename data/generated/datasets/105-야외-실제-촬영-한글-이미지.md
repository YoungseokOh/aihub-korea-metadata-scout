...
## Basic Info
- Dataset Key: `105`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 105`
- Collected At: `2026-03-17T08:40:27.354640+00:00`
...
## File Overview
- File count: `28`
- Total known size: `457.6 GB`
- Representative file paths:
  - `030.야외 실제 촬영 한글 이미지/01.데이터/1.Training/[원천]Training_간판_세로형간판_원천데이터.zip`
  - `030.야외 실제 촬영 한글 이미지/01.데이터/1.Training/[원천]Training_간판_실내간판_원천데이터1.zip`
  - `030.야외 실제 촬영 한글 이미지/01.데이터/1.Training/[원천]Training_간판_실내간판_원천데이터2.zip`
  - `030.야외 실제 촬영 한글 이미지/01.데이터/1.Training/[원천]Training_간판_실내안내판_원천데이터1.zip`
  - `030.야외 실제 촬영 한글 이미지/01.데이터/1.Training/[원천]Training_간판_실내안내판_원천데이터2.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 28개이며 알려진 총 용량은 457.6 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
