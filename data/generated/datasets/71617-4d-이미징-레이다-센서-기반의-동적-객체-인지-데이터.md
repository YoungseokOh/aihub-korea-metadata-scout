...
## Basic Info
- Dataset Key: `71617`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71617`
- Collected At: `2026-03-17T08:41:28.637623+00:00`
...
## File Overview
- File count: `46`
- Total known size: `4.19 TB`
- Representative file paths:
  - `174-1.4D 이미징 레이다 센서 기반의 동적 객체 인지 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z01`
  - `174-1.4D 이미징 레이다 센서 기반의 동적 객체 인지 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z02`
  - `174-1.4D 이미징 레이다 센서 기반의 동적 객체 인지 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z03`
  - `174-1.4D 이미징 레이다 센서 기반의 동적 객체 인지 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z04`
  - `174-1.4D 이미징 레이다 센서 기반의 동적 객체 인지 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z05`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 46개이며 알려진 총 용량은 4.19 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
{
  "dataset_key": 71617,
  "title": "4D 이미징 레이다 센서 기반의 동적 객체 인지 데이터",
  "tags": [
    "computer-vision",
    "객체",
    "4d",
  ...
  "modality_guess": null,
  "file_count": 46,
  "total_size_bytes": 4605978083328,
  "human_size": "4.19 TB",
  "sample_file_paths": [
    "174-1.4D 이미징 레이다 센서 기반의 동적 객체 인지 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z01",
    "174-1.4D 이미징 레이다 센서 기반의 동적 객체 인지 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z02",
