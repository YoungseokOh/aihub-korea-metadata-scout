...
## Basic Info
- Dataset Key: `71791`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71791`
- Collected At: `2026-03-17T08:41:40.565102+00:00`
...
## File Overview
- File count: `4`
- Total known size: `49.0 MB`
- Representative file paths:
  - `108.진로문장완성검사 텍스트 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS.zip`
  - `108.진로문장완성검사 텍스트 데이터/3.개방데이터/1.데이터/Training/02.라벨링데이터/TL.zip`
  - `108.진로문장완성검사 텍스트 데이터/3.개방데이터/1.데이터/Validation/01.원천데이터/VS.zip`
  - `108.진로문장완성검사 텍스트 데이터/3.개방데이터/1.데이터/Validation/02.라벨링데이터/VL.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `nlp/knowledge` 성격의 `text` 자료로 보입니다. 확인 가능한 파일은 4개이며 알려진 총 용량은 49.0 MB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
