...
## Basic Info
- Dataset Key: `71508`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71508`
- Collected At: `2026-03-17T08:41:18.501147+00:00`
...
## File Overview
- File count: `4000`
- Total known size: `946.8 GB`
- Representative file paths:
  - `019.비동일 객체 인식 오류 방지 데이터/01-1.정식개방데이터/Validation/02.라벨링데이터/VL_A49_B01_C050.zip`
  - `019.비동일 객체 인식 오류 방지 데이터/01-1.정식개방데이터/Validation/02.라벨링데이터/VL_A46_B02_C100.zip`
  - `019.비동일 객체 인식 오류 방지 데이터/01-1.정식개방데이터/Validation/02.라벨링데이터/VL_A47_B01_C010.zip`
  - `019.비동일 객체 인식 오류 방지 데이터/01-1.정식개방데이터/Validation/02.라벨링데이터/VL_A47_B01_C020.zip`
  - `019.비동일 객체 인식 오류 방지 데이터/01-1.정식개방데이터/Validation/02.라벨링데이터/VL_A47_B01_C030.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 4000개이며 알려진 총 용량은 946.8 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "modality_guess": null,
  "file_count": 4000,
  "total_size_bytes": 1016663906304,
  "human_size": "946.8 GB",
  "sample_file_paths": [
    "019.비동일 객체 인식 오류 방지 데이터/01-1.정식개방데이터/Validation/02.라벨링데이터/VL_A49_B01_C050.zip",
    "019.비동일 객체 인식 오류 방지 데이터/01-1.정식개방데이터/Validation/02.라벨링데이터/VL_A46_B02_C100.zip",
