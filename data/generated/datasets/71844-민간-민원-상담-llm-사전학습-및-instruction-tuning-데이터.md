...
## Basic Info
- Dataset Key: `71844`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71844`
- Collected At: `2026-03-17T08:41:43.737828+00:00`
...
## File Overview
- File count: `24`
- Total known size: `208.2 MB`
- Representative file paths:
  - `23.민간 민원 상담 LLM 사전학습 및 Instruction Tuning 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_액티벤처.zip`
  - `23.민간 민원 상담 LLM 사전학습 및 Instruction Tuning 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_엘지유플러스.zip`
  - `23.민간 민원 상담 LLM 사전학습 및 Instruction Tuning 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_하나카드.zip`
  - `23.민간 민원 상담 LLM 사전학습 및 Instruction Tuning 데이터/3.개방데이터/1.데이터/Training/02.라벨링데이터/TL_액티벤처_분류.zip`
  - `23.민간 민원 상담 LLM 사전학습 및 Instruction Tuning 데이터/3.개방데이터/1.데이터/Training/02.라벨링데이터/TL_액티벤처_요약.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `speech/dialogue` 성격의 `multimodal` 자료로 보입니다. 확인 가능한 파일은 24개이며 알려진 총 용량은 208.2 MB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
{
  "dataset_key": 71844,
  "title": "민간 민원 상담 LLM 사전학습 및 Instruction Tuning 데이터",
  "tags": [
    "speech/dialogue",
    "multimodal",
    "legal/compliance",
