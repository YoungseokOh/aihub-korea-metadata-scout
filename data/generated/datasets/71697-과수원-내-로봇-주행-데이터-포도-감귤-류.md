...
## Basic Info
- Dataset Key: `71697`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71697`
- Collected At: `2026-03-17T08:41:32.811100+00:00`
...
## File Overview
- File count: `61`
- Total known size: `5.50 TB`
- Representative file paths:
  - `062.과수원 내 로봇 주행 데이터_포도, 감귤 류/3.개방데이터/1.데이터/Other/Other.z01`
  - `062.과수원 내 로봇 주행 데이터_포도, 감귤 류/3.개방데이터/1.데이터/Other/Other.z02`
  - `062.과수원 내 로봇 주행 데이터_포도, 감귤 류/3.개방데이터/1.데이터/Other/Other.z03`
  - `062.과수원 내 로봇 주행 데이터_포도, 감귤 류/3.개방데이터/1.데이터/Other/Other.z04`
  - `062.과수원 내 로봇 주행 데이터_포도, 감귤 류/3.개방데이터/1.데이터/Other/Other.z05`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `robotics` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 61개이며 알려진 총 용량은 5.50 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "modality_guess": null,
  "file_count": 61,
  "total_size_bytes": 6051396059136,
  "human_size": "5.50 TB",
  "sample_file_paths": [
    "062.과수원 내 로봇 주행 데이터_포도, 감귤 류/3.개방데이터/1.데이터/Other/Other.z01",
    "062.과수원 내 로봇 주행 데이터_포도, 감귤 류/3.개방데이터/1.데이터/Other/Other.z02",
