...
## Basic Info
- Dataset Key: `654`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 654`
- Collected At: `2026-03-17T08:40:56.435379+00:00`
...
## File Overview
- File count: `34`
- Total known size: `1005.9 GB`
- Representative file paths:
  - `087.도로 로드마크 인식을 위한 주행 영상 데이터/01.데이터/1.Training/라벨링데이터/TL1.01_온전1.zip`
  - `087.도로 로드마크 인식을 위한 주행 영상 데이터/01.데이터/1.Training/라벨링데이터/TL10.03_비온전(환경)2.zip`
  - `087.도로 로드마크 인식을 위한 주행 영상 데이터/01.데이터/1.Training/라벨링데이터/TL11.03_비온전(환경)3.zip`
  - `087.도로 로드마크 인식을 위한 주행 영상 데이터/01.데이터/1.Training/라벨링데이터/TL12.03_비온전(환경)4.zip`
  - `087.도로 로드마크 인식을 위한 주행 영상 데이터/01.데이터/1.Training/라벨링데이터/TL13.04_비온전(복합).zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 34개이며 알려진 총 용량은 1005.9 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
{
  "dataset_key": 654,
  "title": "도로 로드마크 인식을 위한 주행 영상 데이터",
  "tags": [
    "computer-vision",
    "video",
    "영상",
  ...
  "file_count": 34,
  "total_size_bytes": 1080108777472,
  "human_size": "1005.9 GB",
  "sample_file_paths": [
    "087.도로 로드마크 인식을 위한 주행 영상 데이터/01.데이터/1.Training/라벨링데이터/TL1.01_온전1.zip",
    "087.도로 로드마크 인식을 위한 주행 영상 데이터/01.데이터/1.Training/라벨링데이터/TL10.03_비온전(환경)2.zip",
    "087.도로 로드마크 인식을 위한 주행 영상 데이터/01.데이터/1.Training/라벨링데이터/TL11.03_비온전(환경)3.zip",
