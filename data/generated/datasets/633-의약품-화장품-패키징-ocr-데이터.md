...
## Basic Info
- Dataset Key: `633`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 633`
- Collected At: `2026-03-17T08:40:54.845502+00:00`
...
## File Overview
- File count: `6`
- Total known size: `151.6 GB`
- Representative file paths:
  - `056.의약품, 화장품 패키징 OCR 데이터/01.데이터/1. Training/라벨링데이터/TL1.zip`
  - `056.의약품, 화장품 패키징 OCR 데이터/01.데이터/1. Training/라벨링데이터/TL2.zip`
  - `056.의약품, 화장품 패키징 OCR 데이터/01.데이터/1. Training/원천데이터/TS1.zip`
  - `056.의약품, 화장품 패키징 OCR 데이터/01.데이터/1. Training/원천데이터/TS2.zip`
  - `056.의약품, 화장품 패키징 OCR 데이터/01.데이터/2. Validation/라벨링데이터/VL1.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `ocr/document` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 6개이며 알려진 총 용량은 151.6 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
