...
## Basic Info
- Dataset Key: `181`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 181`
- Collected At: `2026-03-17T08:40:35.637772+00:00`
...
## File Overview
- File count: `69`
- Total known size: `3.05 TB`
- Representative file paths:
  - `012.도로환경파노라마/1.Training/원천데이터/23-01.zip`
  - `012.도로환경파노라마/1.Training/원천데이터/23-02.zip`
  - `012.도로환경파노라마/1.Training/원천데이터/24-01.zip`
  - `012.도로환경파노라마/1.Training/원천데이터/24-02.zip`
  - `012.도로환경파노라마/1.Training/원천데이터/25-01.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 69개이며 알려진 총 용량은 3.05 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 69,
  "total_size_bytes": 3350119579648,
  "human_size": "3.05 TB",
  "sample_file_paths": [
    "012.도로환경파노라마/1.Training/원천데이터/23-01.zip",
    "012.도로환경파노라마/1.Training/원천데이터/23-02.zip",
