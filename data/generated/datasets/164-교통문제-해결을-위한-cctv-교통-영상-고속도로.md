...
## Basic Info
- Dataset Key: `164`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 164`
- Collected At: `2026-03-17T08:40:33.617360+00:00`
...
## File Overview
- File count: `95`
- Total known size: `1.28 TB`
- Representative file paths:
  - `100.교통문제 해결을 위한 CCTV 교통 데이터(고속도로)/01.데이터/1.Training/원천데이터/폴리곤세그멘테이션/5-1.전북.zip`
  - `100.교통문제 해결을 위한 CCTV 교통 데이터(고속도로)/01.데이터/1.Training/원천데이터/폴리곤세그멘테이션/5-2.전북.zip`
  - `100.교통문제 해결을 위한 CCTV 교통 데이터(고속도로)/01.데이터/1.Training/원천데이터/폴리곤세그멘테이션/5-3.전북.zip`
  - `100.교통문제 해결을 위한 CCTV 교통 데이터(고속도로)/01.데이터/1.Training/원천데이터/폴리곤세그멘테이션/5-4.전북.zip`
  - `100.교통문제 해결을 위한 CCTV 교통 데이터(고속도로)/01.데이터/1.Training/원천데이터/폴리곤세그멘테이션/5-5.전북.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `education` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 95개이며 알려진 총 용량은 1.28 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
{
  "dataset_key": 164,
  "title": "교통문제 해결을 위한 CCTV 교통 영상(고속도로)",
  "tags": [
    "education",
    "video",
    "computer-vision",
  ...
  "file_count": 95,
  "total_size_bytes": 1408160308224,
  "human_size": "1.28 TB",
  "sample_file_paths": [
    "100.교통문제 해결을 위한 CCTV 교통 데이터(고속도로)/01.데이터/1.Training/원천데이터/폴리곤세그멘테이션/5-1.전북.zip",
    "100.교통문제 해결을 위한 CCTV 교통 데이터(고속도로)/01.데이터/1.Training/원천데이터/폴리곤세그멘테이션/5-2.전북.zip",
