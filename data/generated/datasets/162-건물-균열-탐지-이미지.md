...
## Basic Info
- Dataset Key: `162`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 162`
- Collected At: `2026-03-17T08:40:33.397744+00:00`
...
- Representative file paths:
  - `112.건물 균열 탐지드론 개발을 위한 이미지/01.데이터/1.Training/원천데이터/강재_강재손상_원천_11.zip`
  - `112.건물 균열 탐지드론 개발을 위한 이미지/01.데이터/1.Training/원천데이터/콘크리트_콘크리트균열_원천_08.zip`
  - `112.건물 균열 탐지드론 개발을 위한 이미지/01.데이터/1.Training/원천데이터/콘크리트_콘크리트균열_원천_09.zip`
  - `112.건물 균열 탐지드론 개발을 위한 이미지/01.데이터/1.Training/원천데이터/콘크리트_콘크리트균열_원천_10.zip`
  - `112.건물 균열 탐지드론 개발을 위한 이미지/01.데이터/1.Training/원천데이터/콘크리트_콘크리트균열_원천_11.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 148개이며 알려진 총 용량은 3.70 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 148,
  "total_size_bytes": 4063375654912,
  "human_size": "3.70 TB",
  "sample_file_paths": [
    "112.건물 균열 탐지드론 개발을 위한 이미지/01.데이터/1.Training/원천데이터/강재_강재손상_원천_11.zip",
    "112.건물 균열 탐지드론 개발을 위한 이미지/01.데이터/1.Training/원천데이터/콘크리트_콘크리트균열_원천_08.zip",
