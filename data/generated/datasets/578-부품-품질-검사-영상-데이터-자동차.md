...
## Basic Info
- Dataset Key: `578`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 578`
- Collected At: `2026-03-17T08:40:49.889236+00:00`
...
## File Overview
- File count: `45`
- Total known size: `452.7 GB`
- Representative file paths:
  - `162.부품 품질 검사 영상 데이터(자동차)/01.데이터/2.Validation/원천데이터/VS_1.도어.tar`
  - `162.부품 품질 검사 영상 데이터(자동차)/01.데이터/2.Validation/원천데이터/VS_10.헤드 램프.tar`
  - `162.부품 품질 검사 영상 데이터(자동차)/01.데이터/2.Validation/원천데이터/VS_11.휀더.tar`
  - `162.부품 품질 검사 영상 데이터(자동차)/01.데이터/2.Validation/원천데이터/VS_2.라디에이터 그릴.tar`
  - `162.부품 품질 검사 영상 데이터(자동차)/01.데이터/2.Validation/원천데이터/VS_3.루프사이드.tar`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 45개이며 알려진 총 용량은 452.7 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
