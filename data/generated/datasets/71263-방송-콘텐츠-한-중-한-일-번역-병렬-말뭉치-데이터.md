...
## Basic Info
- Dataset Key: `71263`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71263`
- Collected At: `2026-03-17T08:40:57.019104+00:00`
...
## File Overview
- File count: `10`
- Total known size: `354.0 MB`
- Representative file paths:
  - `157.방송 콘텐츠 한-중, 한-일 번역 병렬 말뭉치 데이터/01.데이터/1.Training/라벨링데이터/TL1.zip`
  - `157.방송 콘텐츠 한-중, 한-일 번역 병렬 말뭉치 데이터/01.데이터/1.Training/라벨링데이터/TL2.zip`
  - `157.방송 콘텐츠 한-중, 한-일 번역 병렬 말뭉치 데이터/01.데이터/1.Training/라벨링데이터/TL3.zip`
  - `157.방송 콘텐츠 한-중, 한-일 번역 병렬 말뭉치 데이터/01.데이터/1.Training/라벨링데이터/TL4.zip`
  - `157.방송 콘텐츠 한-중, 한-일 번역 병렬 말뭉치 데이터/01.데이터/1.Training/원천데이터/TS1.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `nlp/knowledge` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 10개이며 알려진 총 용량은 354.0 MB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
{
  "dataset_key": 71263,
  "title": "방송 콘텐츠 한-중, 한-일 번역 병렬 말뭉치 데이터",
  "tags": [
    "nlp/knowledge",
    "video",
    "text",
