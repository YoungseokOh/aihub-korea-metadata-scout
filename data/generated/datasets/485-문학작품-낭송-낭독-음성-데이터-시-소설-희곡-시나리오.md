# 문학작품 낭송․낭독 음성 데이터(시, 소설, 희곡, 시나리오)

## Basic Info
- Dataset Key: `485`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 485`
- Collected At: `2026-03-17T08:40:42.700351+00:00`
- Parse Status: `success`
- Inferred tags:
  - `speech/dialogue`
  - `audio`
  - `음성`
  - `낭독`
  - `낭송`
  - `문학작품`
  - `소설`
  - `시나리오`
  - `희곡`
  - `tl1`
  - `ts`
  - `vl2`
  - `vs`
  - `검증레이블`
  - `학습레이블`

## File Overview
- File count: `4`
- Total known size: `94.0 GB`
- Representative file paths:
  - `158.문학작품 낭송, 낭독 음성 데이터(시, 소설, 희곡, 시나리오)/01.데이터/2.Validation/원천데이터/VS.zip`
  - `158.문학작품 낭송, 낭독 음성 데이터(시, 소설, 희곡, 시나리오)/01.데이터/2.Validation/라벨링데이터/VL2.검증레이블.zip`
  - `158.문학작품 낭송, 낭독 음성 데이터(시, 소설, 희곡, 시나리오)/01.데이터/1.Training/원천데이터/TS.zip`
  - `158.문학작품 낭송, 낭독 음성 데이터(시, 소설, 희곡, 시나리오)/01.데이터/1.Training/라벨링데이터/TL1.학습레이블.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `speech/dialogue` 성격의 `audio` 자료로 보입니다. 확인 가능한 파일은 4개이며 알려진 총 용량은 94.0 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).

## Potential Use Cases
- 콜센터 품질 분석
- 음성 상담 요약 및 QA 도우미
- 도메인 특화 음성 평가 벤치마크

## Business Model Exploration
- likely customer types:
  - 컨택센터 운영팀
  - 음성 분석 SaaS 팀
  - 고객지원 플랫폼 팀
- possible product ideas:
  - 콜센터 품질 분석
  - 음성 상담 요약 및 QA 도우미
  - 도메인 특화 음성 평가 벤치마크
- monetization paths:
  - 좌석 기반 SaaS
  - 통화량 기반 사용량 과금
  - 평가 리포트 패키지
- why it could be interesting commercially:
  - 메타데이터만 봐도 활용 장면이 비교적 선명하게 떠오르는 편입니다.
  - 구조가 드러난 파일 트리는 초기 타당성 검토 속도를 높여 줍니다.
- why it might fail commercially:
  - 실제 내용 품질이 기대보다 낮으면 사업화 가정이 쉽게 무너질 수 있습니다.
  - 메타데이터 기반 해석이라 현업 니즈와 어긋날 가능성을 열어 둬야 합니다.

## Practicality Assessment
- training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다.
- 라벨링 데이터와 원천 데이터가 모두 보여서 지도학습 준비도가 상대적으로 높아 보입니다.
- 전처리 자동화가 있으면 시범 적용은 가능하지만 운영화까지는 추가 정리가 필요해 보입니다.
- 법률·정책 적합성은 별도 확인이 필요합니다.
- legal/policy caution placeholder: License and policy suitability must be validated separately.

## Scores
- Opportunity score: `7` / 10
- Build difficulty: `5` / 10
- Data readiness: `8` / 10
- Score reasons:
  - 상업적 수요가 반복적으로 발생하는 문제 영역으로 보입니다.
  - 수십 GB 규모라 로컬 처리 부담이 있습니다.
  - 파일 구조가 단순해 보여 구조 파악이 쉽습니다.
  - 라벨링 데이터가 보여 활용 가설을 세우기 쉽습니다.
  - 학습/검증 분리가 보입니다.

## Raw Metadata Snapshot
```json
{
  "dataset_key": 485,
  "title": "문학작품 낭송․낭독 음성 데이터(시, 소설, 희곡, 시나리오)",
  "tags": [
    "speech/dialogue",
    "audio",
    "음성",
    "낭독",
    "낭송",
    "문학작품",
    "소설",
    "시나리오",
    "희곡",
    "tl1"
  ],
  "category_guess": "speech/dialogue",
  "modality_guess": "audio",
  "file_count": 4,
  "total_size_bytes": 100961091584,
  "human_size": "94.0 GB",
  "sample_file_paths": [
    "158.문학작품 낭송, 낭독 음성 데이터(시, 소설, 희곡, 시나리오)/01.데이터/2.Validation/원천데이터/VS.zip",
    "158.문학작품 낭송, 낭독 음성 데이터(시, 소설, 희곡, 시나리오)/01.데이터/2.Validation/라벨링데이터/VL2.검증레이블.zip",
    "158.문학작품 낭송, 낭독 음성 데이터(시, 소설, 희곡, 시나리오)/01.데이터/1.Training/원천데이터/TS.zip",
    "158.문학작품 낭송, 낭독 음성 데이터(시, 소설, 희곡, 시나리오)/01.데이터/1.Training/라벨링데이터/TL1.학습레이블.zip"
  ],
  "parse_status": "success",
  "parse_warnings": [],
  "notices": []
}
```