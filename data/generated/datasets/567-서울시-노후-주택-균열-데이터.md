...
## Basic Info
- Dataset Key: `567`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 567`
- Collected At: `2026-03-17T08:40:48.732605+00:00`
...
## File Overview
- File count: `32`
- Total known size: `429.6 GB`
- Representative file paths:
  - `189.서울시 노후 주택 균열 데이터/01.데이터/1.Training/라벨링데이터/TL_다세대주택.egg`
  - `189.서울시 노후 주택 균열 데이터/01.데이터/1.Training/라벨링데이터/TL_단독주택.egg`
  - `189.서울시 노후 주택 균열 데이터/01.데이터/1.Training/라벨링데이터/TL_비주거용주택.zip`
  - `189.서울시 노후 주택 균열 데이터/01.데이터/1.Training/라벨링데이터/TL_아파트.egg`
  - `189.서울시 노후 주택 균열 데이터/01.데이터/1.Training/라벨링데이터/TL_연립주택.egg`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 32개이며 알려진 총 용량은 429.6 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
