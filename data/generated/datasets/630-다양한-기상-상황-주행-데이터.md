...
## Basic Info
- Dataset Key: `630`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 630`
- Collected At: `2026-03-17T08:40:54.662221+00:00`
...
- Representative file paths:
  - `085.다양한 기상 상황 주행 데이터/01.데이터/2.Validation/원천데이터/VS_Snowy.zip`
  - `085.다양한 기상 상황 주행 데이터/01.데이터/2.Validation/원천데이터/VS_Hazy.zip`
  - `085.다양한 기상 상황 주행 데이터/01.데이터/2.Validation/원천데이터/VS_Normal.zip`
  - `085.다양한 기상 상황 주행 데이터/01.데이터/2.Validation/원천데이터/VS_Rainy.zip`
  - `085.다양한 기상 상황 주행 데이터/01.데이터/2.Validation/라벨링데이터/VL_Hazy.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 16개이며 알려진 총 용량은 150.5 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
