...
## Basic Info
- Dataset Key: `176`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 176`
- Collected At: `2026-03-17T08:40:35.080133+00:00`
...
## File Overview
- File count: `46`
- Total known size: `996.8 GB`
- Representative file paths:
  - `106.화재 발생 예측 데이터(연기 동영상)/01.데이터/1.Training/원천데이터(zip)/유사씬_2.zip`
  - `106.화재 발생 예측 데이터(연기 동영상)/01.데이터/1.Training/원천데이터(zip)/유사씬_3.zip`
  - `106.화재 발생 예측 데이터(연기 동영상)/01.데이터/1.Training/원천데이터(zip)/유사씬_4.zip`
  - `106.화재 발생 예측 데이터(연기 동영상)/01.데이터/1.Training/원천데이터(zip)/유사씬_5.zip`
  - `106.화재 발생 예측 데이터(연기 동영상)/01.데이터/1.Training/원천데이터(zip)/유사씬_6.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 46개이며 알려진 총 용량은 996.8 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 46,
  "total_size_bytes": 1070327660544,
  "human_size": "996.8 GB",
  "sample_file_paths": [
    "106.화재 발생 예측 데이터(연기 동영상)/01.데이터/1.Training/원천데이터(zip)/유사씬_2.zip",
    "106.화재 발생 예측 데이터(연기 동영상)/01.데이터/1.Training/원천데이터(zip)/유사씬_3.zip",
