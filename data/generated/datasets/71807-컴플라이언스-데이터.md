...
## Basic Info
- Dataset Key: `71807`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71807`
- Collected At: `2026-03-17T08:41:41.383228+00:00`
...
## File Overview
- File count: `37`
- Total known size: `1.64 GB`
- Representative file paths:
  - `17.컴플라이언스 데이터/3.개방데이터/1.데이터/Other/Other.zip`
  - `17.컴플라이언스 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_CP01.zip`
  - `17.컴플라이언스 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_CP02.zip`
  - `17.컴플라이언스 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_CP03.zip`
  - `17.컴플라이언스 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_CP04.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 37개이며 알려진 총 용량은 1.64 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
