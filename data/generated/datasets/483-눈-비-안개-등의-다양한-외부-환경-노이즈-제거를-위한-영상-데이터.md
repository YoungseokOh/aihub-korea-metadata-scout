...
## Basic Info
- Dataset Key: `483`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 483`
- Collected At: `2026-03-17T08:40:42.502393+00:00`
...
## File Overview
- File count: `220`
- Total known size: `1.52 TB`
- Representative file paths:
  - `039.눈, 비, 안개 등의 다양한 외부 환경 노이즈 제거를 위한 영상 데이터/01.데이터/1.Training/원천데이터/TS6_1.비_문화집합시설.zip`
  - `039.눈, 비, 안개 등의 다양한 외부 환경 노이즈 제거를 위한 영상 데이터/01.데이터/1.Training/원천데이터/TS7_1.비_주거시설.zip`
  - `039.눈, 비, 안개 등의 다양한 외부 환경 노이즈 제거를 위한 영상 데이터/01.데이터/1.Training/원천데이터/TS8_1.비_자연환경.zip`
  - `039.눈, 비, 안개 등의 다양한 외부 환경 노이즈 제거를 위한 영상 데이터/01.데이터/1.Training/원천데이터/TS9_2.안개_교통시설.zip`
  - `039.눈, 비, 안개 등의 다양한 외부 환경 노이즈 제거를 위한 영상 데이터/01.데이터/1.Training/원천데이터/TS28_4.눈_문화집합시설.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `education` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 220개이며 알려진 총 용량은 1.52 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
{
  "dataset_key": 483,
  "title": "눈, 비, 안개 등의 다양한 외부 환경 노이즈 제거를 위한 영상 데이터",
  "tags": [
    "education",
    "video",
    "computer-vision",
  ...
  "file_count": 220,
  "total_size_bytes": 1670177782784,
  "human_size": "1.52 TB",
  "sample_file_paths": [
    "039.눈, 비, 안개 등의 다양한 외부 환경 노이즈 제거를 위한 영상 데이터/01.데이터/1.Training/원천데이터/TS6_1.비_문화집합시설.zip",
    "039.눈, 비, 안개 등의 다양한 외부 환경 노이즈 제거를 위한 영상 데이터/01.데이터/1.Training/원천데이터/TS7_1.비_주거시설.zip",
