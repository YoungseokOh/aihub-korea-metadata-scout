...
## Basic Info
- Dataset Key: `71477`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71477`
- Collected At: `2026-03-17T08:41:15.675550+00:00`
...
## File Overview
- File count: `48`
- Total known size: `112.4 GB`
- Representative file paths:
  - `051.자연어 분석 후처리용 과교정 검증 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_TX_CA_1.zip`
  - `051.자연어 분석 후처리용 과교정 검증 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_TX_CA_2.zip`
  - `051.자연어 분석 후처리용 과교정 검증 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_TX_CA_3.zip`
  - `051.자연어 분석 후처리용 과교정 검증 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_TX_ES_1.zip`
  - `051.자연어 분석 후처리용 과교정 검증 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_TX_ES_2.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 48개이며 알려진 총 용량은 112.4 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
