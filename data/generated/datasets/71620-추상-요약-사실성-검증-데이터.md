...
## Basic Info
- Dataset Key: `71620`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71620`
- Collected At: `2026-03-17T08:41:28.837139+00:00`
...
## File Overview
- File count: `83`
- Total known size: `312.8 MB`
- Representative file paths:
  - `157.추상 요약 사실성 검증 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_기계요약문_사실전달형.zip`
  - `157.추상 요약 사실성 검증 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_기계요약문_의견제시형.zip`
  - `157.추상 요약 사실성 검증 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_기계요약문_자유형.zip`
  - `157.추상 요약 사실성 검증 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_사람요약문_사실전달형.zip`
  - `157.추상 요약 사실성 검증 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_사람요약문_의견제시형.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `nlp/knowledge` 성격의 `text` 자료로 보입니다. 확인 가능한 파일은 83개이며 알려진 총 용량은 312.8 MB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
