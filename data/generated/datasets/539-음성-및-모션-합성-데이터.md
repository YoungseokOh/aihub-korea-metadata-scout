...
## Basic Info
- Dataset Key: `539`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 539`
- Collected At: `2026-03-17T08:40:46.425163+00:00`
...
## File Overview
- File count: `110`
- Total known size: `4.05 TB`
- Representative file paths:
  - `010.음성 및 모션 합성 데이터/01.데이터/1.Training/라벨링데이터/TL1.zip`
  - `010.음성 및 모션 합성 데이터/01.데이터/1.Training/라벨링데이터/TL10.zip`
  - `010.음성 및 모션 합성 데이터/01.데이터/1.Training/라벨링데이터/TL11.zip`
  - `010.음성 및 모션 합성 데이터/01.데이터/1.Training/라벨링데이터/TL12.zip`
  - `010.음성 및 모션 합성 데이터/01.데이터/1.Training/라벨링데이터/TL13.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `speech/dialogue` 성격의 `audio` 자료로 보입니다. 확인 가능한 파일은 110개이며 알려진 총 용량은 4.05 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 110,
  "total_size_bytes": 4457951657984,
  "human_size": "4.05 TB",
  "sample_file_paths": [
    "010.음성 및 모션 합성 데이터/01.데이터/1.Training/라벨링데이터/TL1.zip",
    "010.음성 및 모션 합성 데이터/01.데이터/1.Training/라벨링데이터/TL10.zip",
