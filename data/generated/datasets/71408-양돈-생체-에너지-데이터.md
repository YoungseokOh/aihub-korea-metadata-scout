...
## Basic Info
- Dataset Key: `71408`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71408`
- Collected At: `2026-03-17T08:41:09.072777+00:00`
...
- Representative file paths:
  - `276.양돈 생체 에너지 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z01`
  - `276.양돈 생체 에너지 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z02`
  - `276.양돈 생체 에너지 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z03`
  - `276.양돈 생체 에너지 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z04`
  - `276.양돈 생체 에너지 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z05`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 13개이며 알려진 총 용량은 984.3 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "modality_guess": null,
  "file_count": 13,
  "total_size_bytes": 1056869187584,
  "human_size": "984.3 GB",
  "sample_file_paths": [
    "276.양돈 생체 에너지 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z01",
    "276.양돈 생체 에너지 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z02",
