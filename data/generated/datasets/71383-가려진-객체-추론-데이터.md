...
## Basic Info
- Dataset Key: `71383`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71383`
- Collected At: `2026-03-17T08:41:07.593431+00:00`
...
## File Overview
- File count: `46`
- Total known size: `4.08 TB`
- Representative file paths:
  - `195.가려진 객체 추론 데이터/01-1.정식개방데이터/Validation/02.라벨링데이터/VL.zip`
  - `195.가려진 객체 추론 데이터/01-1.정식개방데이터/Validation/01.원천데이터/VS.z01`
  - `195.가려진 객체 추론 데이터/01-1.정식개방데이터/Validation/01.원천데이터/VS.z02`
  - `195.가려진 객체 추론 데이터/01-1.정식개방데이터/Validation/01.원천데이터/VS.z03`
  - `195.가려진 객체 추론 데이터/01-1.정식개방데이터/Validation/01.원천데이터/VS.z04`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 46개이며 알려진 총 용량은 4.08 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "modality_guess": null,
  "file_count": 46,
  "total_size_bytes": 4488528134144,
  "human_size": "4.08 TB",
  "sample_file_paths": [
    "195.가려진 객체 추론 데이터/01-1.정식개방데이터/Validation/02.라벨링데이터/VL.zip",
    "195.가려진 객체 추론 데이터/01-1.정식개방데이터/Validation/01.원천데이터/VS.z01",
