...
## Basic Info
- Dataset Key: `604`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 604`
- Collected At: `2026-03-17T08:40:52.959547+00:00`
...
## File Overview
- File count: `249`
- Total known size: `24.2 TB`
- Representative file paths:
  - `052.다양한 장르의 춤사위 동작 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z114`
  - `052.다양한 장르의 춤사위 동작 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z01`
  - `052.다양한 장르의 춤사위 동작 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z02`
  - `052.다양한 장르의 춤사위 동작 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z03`
  - `052.다양한 장르의 춤사위 동작 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z04`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 249개이며 알려진 총 용량은 24.2 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "modality_guess": null,
  "file_count": 249,
  "total_size_bytes": 26637387169792,
  "human_size": "24.2 TB",
  "sample_file_paths": [
    "052.다양한 장르의 춤사위 동작 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z114",
    "052.다양한 장르의 춤사위 동작 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z01",
