...
## Basic Info
- Dataset Key: `71759`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71759`
- Collected At: `2026-03-17T08:41:37.376181+00:00`
...
## File Overview
- File count: `68`
- Total known size: `5.26 TB`
- Representative file paths:
  - `097.탈춤 동작 데이터/3.개방데이터/1.데이터/Other/Other_1.zip`
  - `097.탈춤 동작 데이터/3.개방데이터/1.데이터/Other/Other_10.zip`
  - `097.탈춤 동작 데이터/3.개방데이터/1.데이터/Other/Other_11.zip`
  - `097.탈춤 동작 데이터/3.개방데이터/1.데이터/Other/Other_12.zip`
  - `097.탈춤 동작 데이터/3.개방데이터/1.데이터/Other/Other_13.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 68개이며 알려진 총 용량은 5.26 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "modality_guess": null,
  "file_count": 68,
  "total_size_bytes": 5786611744768,
  "human_size": "5.26 TB",
  "sample_file_paths": [
    "097.탈춤 동작 데이터/3.개방데이터/1.데이터/Other/Other_1.zip",
    "097.탈춤 동작 데이터/3.개방데이터/1.데이터/Other/Other_10.zip",
