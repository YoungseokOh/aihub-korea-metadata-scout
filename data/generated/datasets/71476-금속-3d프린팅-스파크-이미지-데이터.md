...
## Basic Info
- Dataset Key: `71476`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71476`
- Collected At: `2026-03-17T08:41:15.568129+00:00`
...
## File Overview
- File count: `18`
- Total known size: `1.51 TB`
- Representative file paths:
  - `191.금속 3D프린팅 스파크 이미지 데이터/01-1.정식개방데이터/Other/Other.z01`
  - `191.금속 3D프린팅 스파크 이미지 데이터/01-1.정식개방데이터/Other/Other.zip`
  - `191.금속 3D프린팅 스파크 이미지 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z01`
  - `191.금속 3D프린팅 스파크 이미지 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z02`
  - `191.금속 3D프린팅 스파크 이미지 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z03`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `3d/lidar` 자료로 보입니다. 확인 가능한 파일은 18개이며 알려진 총 용량은 1.51 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 18,
  "total_size_bytes": 1664299827200,
  "human_size": "1.51 TB",
  "sample_file_paths": [
    "191.금속 3D프린팅 스파크 이미지 데이터/01-1.정식개방데이터/Other/Other.z01",
    "191.금속 3D프린팅 스파크 이미지 데이터/01-1.정식개방데이터/Other/Other.zip",
