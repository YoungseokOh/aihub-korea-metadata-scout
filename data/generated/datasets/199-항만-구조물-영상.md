...
## Basic Info
- Dataset Key: `199`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 199`
- Collected At: `2026-03-17T08:40:38.268732+00:00`
...
- Representative file paths:
  - `066.항만 구조물 영상/01.데이터/2.Validation/1.라벨링데이터/bbox/ship/boat_53.zip`
  - `066.항만 구조물 영상/01.데이터/2.Validation/1.라벨링데이터/bbox/ship/boat_55.zip`
  - `066.항만 구조물 영상/01.데이터/2.Validation/1.라벨링데이터/bbox/ship/boat_58.zip`
  - `066.항만 구조물 영상/01.데이터/2.Validation/1.라벨링데이터/bbox/ship/cruise_52.zip`
  - `066.항만 구조물 영상/01.데이터/2.Validation/1.라벨링데이터/bbox/ship/cruise_54.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 636개이며 알려진 총 용량은 2.26 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 636,
  "total_size_bytes": 2481644981778,
  "human_size": "2.26 TB",
  "sample_file_paths": [
    "066.항만 구조물 영상/01.데이터/2.Validation/1.라벨링데이터/bbox/ship/boat_53.zip",
    "066.항만 구조물 영상/01.데이터/2.Validation/1.라벨링데이터/bbox/ship/boat_55.zip",
