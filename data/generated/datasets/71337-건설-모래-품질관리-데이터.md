...
## Basic Info
- Dataset Key: `71337`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71337`
- Collected At: `2026-03-17T08:41:02.660017+00:00`
...
## File Overview
- File count: `25`
- Total known size: `1.80 TB`
- Representative file paths:
  - `264.건설 모래 품질 관리데이터/01-1.정식개방데이터/Validation/02.라벨링데이터/VL_2.모래위험광물분류.zip`
  - `264.건설 모래 품질 관리데이터/01-1.정식개방데이터/Validation/02.라벨링데이터/VL_1.모래입자크기분류.zip`
  - `264.건설 모래 품질 관리데이터/01-1.정식개방데이터/Validation/01.원천데이터/VS_1.모래입자크기분류_1.zip`
  - `264.건설 모래 품질 관리데이터/01-1.정식개방데이터/Validation/01.원천데이터/VS_1.모래입자크기분류_2.zip`
  - `264.건설 모래 품질 관리데이터/01-1.정식개방데이터/Validation/01.원천데이터/VS_2.모래위험광물분류.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 25개이며 알려진 총 용량은 1.80 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "modality_guess": null,
  "file_count": 25,
  "total_size_bytes": 1980384673792,
  "human_size": "1.80 TB",
  "sample_file_paths": [
    "264.건설 모래 품질 관리데이터/01-1.정식개방데이터/Validation/02.라벨링데이터/VL_2.모래위험광물분류.zip",
    "264.건설 모래 품질 관리데이터/01-1.정식개방데이터/Validation/02.라벨링데이터/VL_1.모래입자크기분류.zip",
