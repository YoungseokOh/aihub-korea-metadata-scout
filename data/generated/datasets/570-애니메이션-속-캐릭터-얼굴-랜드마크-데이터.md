...
## Basic Info
- Dataset Key: `570`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 570`
- Collected At: `2026-03-17T08:40:49.094325+00:00`
...
## File Overview
- File count: `828`
- Total known size: `223.7 GB`
- Representative file paths:
  - `190.애니메이션 속 캐릭터 얼굴 랜드마크 데이터/01.데이터/1.Training/라벨링데이터/TL_040_Cloud Bread_S01_208.zip`
  - `190.애니메이션 속 캐릭터 얼굴 랜드마크 데이터/01.데이터/1.Training/라벨링데이터/TL_001_Cloud Bread_S01_100.zip`
  - `190.애니메이션 속 캐릭터 얼굴 랜드마크 데이터/01.데이터/1.Training/라벨링데이터/TL_002_Cloud Bread_S01_104.zip`
  - `190.애니메이션 속 캐릭터 얼굴 랜드마크 데이터/01.데이터/1.Training/라벨링데이터/TL_003_Cloud Bread_S01_111.zip`
  - `190.애니메이션 속 캐릭터 얼굴 랜드마크 데이터/01.데이터/1.Training/라벨링데이터/TL_004_Cloud Bread_S01_112.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 828개이며 알려진 총 용량은 223.7 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
