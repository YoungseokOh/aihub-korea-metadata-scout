# 자연 발화 동영상-인체 3D 포즈 데이터

## Basic Info
- Dataset Key: `71815`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71815`
- Collected At: `2026-03-17T08:41:41.950432+00:00`
...
## File Overview
- File count: `243`
- Total known size: `23.3 TB`
- Representative file paths:
  - `34.자연 발화 동영상-인체 3D 포즈 데이터/3.개방데이터/1.데이터/Other/Other.z01`
  - `34.자연 발화 동영상-인체 3D 포즈 데이터/3.개방데이터/1.데이터/Other/Other.z02`
  - `34.자연 발화 동영상-인체 3D 포즈 데이터/3.개방데이터/1.데이터/Other/Other.z03`
  - `34.자연 발화 동영상-인체 3D 포즈 데이터/3.개방데이터/1.데이터/Other/Other.z04`
  - `34.자연 발화 동영상-인체 3D 포즈 데이터/3.개방데이터/1.데이터/Other/Other.z05`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `speech/dialogue` 성격의 `3d/lidar` 자료로 보입니다. 확인 가능한 파일은 243개이며 알려진 총 용량은 23.3 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 243,
  "total_size_bytes": 25662649794560,
  "human_size": "23.3 TB",
  "sample_file_paths": [
    "34.자연 발화 동영상-인체 3D 포즈 데이터/3.개방데이터/1.데이터/Other/Other.z01",
    "34.자연 발화 동영상-인체 3D 포즈 데이터/3.개방데이터/1.데이터/Other/Other.z02",
