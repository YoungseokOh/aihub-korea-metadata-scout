...
## Basic Info
- Dataset Key: `71851`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71851`
- Collected At: `2026-03-17T08:41:44.215775+00:00`
...
## File Overview
- File count: `80`
- Total known size: `2.28 TB`
- Representative file paths:
  - `45.사고위험 환경에서의 운전습관 데이터/3.개방데이터/1.데이터/Other/Other.zip`
  - `45.사고위험 환경에서의 운전습관 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_기타구간_CAN 데이터.zip`
  - `45.사고위험 환경에서의 운전습관 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_기타구간_운전자 운전행동 이미지.zip`
  - `45.사고위험 환경에서의 운전습관 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_기타구간_운전자 운전행동.zip`
  - `45.사고위험 환경에서의 운전습관 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_기타구간_주행 환경정보 이미지.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 80개이며 알려진 총 용량은 2.28 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 80,
  "total_size_bytes": 2507928804352,
  "human_size": "2.28 TB",
  "sample_file_paths": [
    "45.사고위험 환경에서의 운전습관 데이터/3.개방데이터/1.데이터/Other/Other.zip",
    "45.사고위험 환경에서의 운전습관 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_기타구간_CAN 데이터.zip",
