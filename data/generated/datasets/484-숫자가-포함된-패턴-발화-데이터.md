...
## Basic Info
- Dataset Key: `484`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 484`
- Collected At: `2026-03-17T08:40:42.607760+00:00`
...
## File Overview
- File count: `129`
- Total known size: `1.07 TB`
- Representative file paths:
  - `159.숫자가 포함된 패턴 발화 데이터/01.데이터/1.Training/라벨링데이터/TL_13.신분증번호.zip`
  - `159.숫자가 포함된 패턴 발화 데이터/01.데이터/1.Training/라벨링데이터/TL_12.스포츠.zip`
  - `159.숫자가 포함된 패턴 발화 데이터/01.데이터/1.Training/라벨링데이터/TL_11.사투리.zip`
  - `159.숫자가 포함된 패턴 발화 데이터/01.데이터/1.Training/라벨링데이터/TL_5.금융-은행.zip`
  - `159.숫자가 포함된 패턴 발화 데이터/01.데이터/1.Training/라벨링데이터/TL_4.교통정보.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `speech/dialogue` 성격의 `audio` 자료로 보입니다. 확인 가능한 파일은 129개이며 알려진 총 용량은 1.07 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 129,
  "total_size_bytes": 1175007933440,
  "human_size": "1.07 TB",
  "sample_file_paths": [
    "159.숫자가 포함된 패턴 발화 데이터/01.데이터/1.Training/라벨링데이터/TL_13.신분증번호.zip",
    "159.숫자가 포함된 패턴 발화 데이터/01.데이터/1.Training/라벨링데이터/TL_12.스포츠.zip",
