...
## Basic Info
- Dataset Key: `557`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 557`
- Collected At: `2026-03-17T08:40:48.021198+00:00`
...
- Representative file paths:
  - `144.지자체 도로 정비 AI 데이터/01.데이터/1.Training/라벨링데이터/TL01_지자체도로정비AI데이터_CASE1(도로).zip`
  - `144.지자체 도로 정비 AI 데이터/01.데이터/1.Training/라벨링데이터/TL02_지자체도로정비AI데이터_CASE1(도로).zip`
  - `144.지자체 도로 정비 AI 데이터/01.데이터/1.Training/라벨링데이터/TL03_지자체도로정비AI데이터_CASE1(도로).zip`
  - `144.지자체 도로 정비 AI 데이터/01.데이터/1.Training/라벨링데이터/TL04_지자체도로정비AI데이터_CASE1(도로).zip`
  - `144.지자체 도로 정비 AI 데이터/01.데이터/1.Training/라벨링데이터/TL05_지자체도로정비AI데이터_CASE1(도로).zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 61개이며 알려진 총 용량은 3.95 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 61,
  "total_size_bytes": 4341585936384,
  "human_size": "3.95 TB",
  "sample_file_paths": [
    "144.지자체 도로 정비 AI 데이터/01.데이터/1.Training/라벨링데이터/TL01_지자체도로정비AI데이터_CASE1(도로).zip",
    "144.지자체 도로 정비 AI 데이터/01.데이터/1.Training/라벨링데이터/TL02_지자체도로정비AI데이터_CASE1(도로).zip",
