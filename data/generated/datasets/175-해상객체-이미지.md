...
## Basic Info
- Dataset Key: `175`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 175`
- Collected At: `2026-03-17T08:40:34.984357+00:00`
...
## File Overview
- File count: `257`
- Total known size: `4.39 TB`
- Representative file paths:
  - `135.해상 객체 이미지/01.데이터/2.Validation/라벨링데이터/서해_장항1구역_SEG.zip`
  - `135.해상 객체 이미지/01.데이터/2.Validation/라벨링데이터/서해_대천항3구역_BOX.zip`
  - `135.해상 객체 이미지/01.데이터/2.Validation/라벨링데이터/서해_장항1구역_BOX.zip`
  - `135.해상 객체 이미지/01.데이터/2.Validation/라벨링데이터/남해_여수항1구역_BOX.zip`
  - `135.해상 객체 이미지/01.데이터/2.Validation/라벨링데이터/남해_여수항4구역_BOX.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 257개이며 알려진 총 용량은 4.39 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 257,
  "total_size_bytes": 4826512429056,
  "human_size": "4.39 TB",
  "sample_file_paths": [
    "135.해상 객체 이미지/01.데이터/2.Validation/라벨링데이터/서해_장항1구역_SEG.zip",
    "135.해상 객체 이미지/01.데이터/2.Validation/라벨링데이터/서해_대천항3구역_BOX.zip",
