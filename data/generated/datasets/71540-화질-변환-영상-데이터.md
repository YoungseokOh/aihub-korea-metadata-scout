...
## Basic Info
- Dataset Key: `71540`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71540`
- Collected At: `2026-03-17T08:41:22.405887+00:00`
...
## File Overview
- File count: `168`
- Total known size: `1.11 TB`
- Representative file paths:
  - `029.화질 변환 영상 데이터/01-1.정식개방데이터/Training/02.라벨링데이터/TL_실외_O_High_L01.zip`
  - `029.화질 변환 영상 데이터/01-1.정식개방데이터/Training/02.라벨링데이터/TL_실내_I_Eye_H01.zip`
  - `029.화질 변환 영상 데이터/01-1.정식개방데이터/Training/02.라벨링데이터/TL_실내_I_Eye_H02.zip`
  - `029.화질 변환 영상 데이터/01-1.정식개방데이터/Training/02.라벨링데이터/TL_실내_I_Eye_H03.zip`
  - `029.화질 변환 영상 데이터/01-1.정식개방데이터/Training/02.라벨링데이터/TL_실내_I_Eye_H04.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 168개이며 알려진 총 용량은 1.11 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 168,
  "total_size_bytes": 1216188776448,
  "human_size": "1.11 TB",
  "sample_file_paths": [
    "029.화질 변환 영상 데이터/01-1.정식개방데이터/Training/02.라벨링데이터/TL_실외_O_High_L01.zip",
    "029.화질 변환 영상 데이터/01-1.정식개방데이터/Training/02.라벨링데이터/TL_실내_I_Eye_H01.zip",
