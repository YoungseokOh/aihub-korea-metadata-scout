...
## Basic Info
- Dataset Key: `182`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 182`
- Collected At: `2026-03-17T08:40:35.740129+00:00`
...
## File Overview
- File count: `182`
- Total known size: `1.85 TB`
- Representative file paths:
  - `064.드론 이동체 인지 영상(도로 고정)/01.데이터/1.Training/라벨링데이터/03 survivor/조난자_안산01.zip`
  - `064.드론 이동체 인지 영상(도로 고정)/01.데이터/1.Training/라벨링데이터/03 survivor/조난자_안산02.zip`
  - `064.드론 이동체 인지 영상(도로 고정)/01.데이터/1.Training/라벨링데이터/03 survivor/조난자_안산03.zip`
  - `064.드론 이동체 인지 영상(도로 고정)/01.데이터/1.Training/라벨링데이터/03 survivor/조난자_의왕01.zip`
  - `064.드론 이동체 인지 영상(도로 고정)/01.데이터/1.Training/라벨링데이터/03 survivor/조난자_의왕02.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 182개이며 알려진 총 용량은 1.85 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 182,
  "total_size_bytes": 2029496827904,
  "human_size": "1.85 TB",
  "sample_file_paths": [
    "064.드론 이동체 인지 영상(도로 고정)/01.데이터/1.Training/라벨링데이터/03 survivor/조난자_안산01.zip",
    "064.드론 이동체 인지 영상(도로 고정)/01.데이터/1.Training/라벨링데이터/03 survivor/조난자_안산02.zip",
