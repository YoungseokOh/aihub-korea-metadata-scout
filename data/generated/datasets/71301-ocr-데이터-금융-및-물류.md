...
## Basic Info
- Dataset Key: `71301`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71301`
- Collected At: `2026-03-17T08:40:59.473138+00:00`
...
## File Overview
- File count: `136`
- Total known size: `165.2 GB`
- Representative file paths:
  - `025.OCR 데이터(금융 및 물류)/01-1.정식개방데이터/Training/01.원천데이터/TS_물류_5.기타_ET03.zip`
  - `025.OCR 데이터(금융 및 물류)/01-1.정식개방데이터/Training/01.원천데이터/TS_금융_1.은행_1-1.신고서.zip`
  - `025.OCR 데이터(금융 및 물류)/01-1.정식개방데이터/Training/01.원천데이터/TS_금융_1.은행_1-2.신청서.zip`
  - `025.OCR 데이터(금융 및 물류)/01-1.정식개방데이터/Training/01.원천데이터/TS_금융_1.은행_1-3.확인서.zip`
  - `025.OCR 데이터(금융 및 물류)/01-1.정식개방데이터/Training/01.원천데이터/TS_금융_1.은행_1-4.위임장.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `ocr/document` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 136개이며 알려진 총 용량은 165.2 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
