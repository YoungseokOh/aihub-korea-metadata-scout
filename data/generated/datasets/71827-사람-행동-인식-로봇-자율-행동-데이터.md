# 사람 행동 인식 로봇 자율 행동 데이터

## Basic Info
- Dataset Key: `71827`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71827`
- Collected At: `2026-03-17T08:41:42.737493+00:00`
...
## File Overview
- File count: `47`
- Total known size: `1.17 TB`
- Representative file paths:
  - `40.사람 행동 인식 로봇 자율 행동 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_01. 교통시설.zip`
  - `40.사람 행동 인식 로봇 자율 행동 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_02. 의료시설_1.zip`
  - `40.사람 행동 인식 로봇 자율 행동 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_02. 의료시설_2.zip`
  - `40.사람 행동 인식 로봇 자율 행동 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_03. 다중시설_1.zip`
  - `40.사람 행동 인식 로봇 자율 행동 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_03. 다중시설_2.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `education` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 47개이며 알려진 총 용량은 1.17 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "modality_guess": null,
  "file_count": 47,
  "total_size_bytes": 1287133008896,
  "human_size": "1.17 TB",
  "sample_file_paths": [
    "40.사람 행동 인식 로봇 자율 행동 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_01. 교통시설.zip",
    "40.사람 행동 인식 로봇 자율 행동 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_02. 의료시설_1.zip",
