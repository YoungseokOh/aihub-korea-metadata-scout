...
## Basic Info
- Dataset Key: `71314`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71314`
- Collected At: `2026-03-17T08:41:00.718165+00:00`
...
## File Overview
- File count: `32`
- Total known size: `559.3 GB`
- Representative file paths:
  - `030.방송콘텐츠 대화체 음성인식 데이터/01-1.정식개방데이터/Validation/02.라벨링데이터/VL_토크쇼.zip`
  - `030.방송콘텐츠 대화체 음성인식 데이터/01-1.정식개방데이터/Validation/02.라벨링데이터/VL_교양.zip`
  - `030.방송콘텐츠 대화체 음성인식 데이터/01-1.정식개방데이터/Validation/02.라벨링데이터/VL_문화.zip`
  - `030.방송콘텐츠 대화체 음성인식 데이터/01-1.정식개방데이터/Validation/02.라벨링데이터/VL_시사.zip`
  - `030.방송콘텐츠 대화체 음성인식 데이터/01-1.정식개방데이터/Validation/02.라벨링데이터/VL_예능.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `speech/dialogue` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 32개이며 알려진 총 용량은 559.3 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
