...
## Basic Info
- Dataset Key: `551`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 551`
- Collected At: `2026-03-17T08:40:47.563377+00:00`
...
## File Overview
- File count: `88`
- Total known size: `5.46 TB`
- Representative file paths:
  - `046.실생활 투명 객체 3D 데이터/01.데이터/1.Training/1_원천데이터/TS01.zip`
  - `046.실생활 투명 객체 3D 데이터/01.데이터/1.Training/1_원천데이터/TS02.zip`
  - `046.실생활 투명 객체 3D 데이터/01.데이터/1.Training/1_원천데이터/TS03.zip`
  - `046.실생활 투명 객체 3D 데이터/01.데이터/1.Training/1_원천데이터/TS04.zip`
  - `046.실생활 투명 객체 3D 데이터/01.데이터/1.Training/1_원천데이터/TS05.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `3d/lidar` 자료로 보입니다. 확인 가능한 파일은 88개이며 알려진 총 용량은 5.46 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 88,
  "total_size_bytes": 5998685192192,
  "human_size": "5.46 TB",
  "sample_file_paths": [
    "046.실생활 투명 객체 3D 데이터/01.데이터/1.Training/1_원천데이터/TS01.zip",
    "046.실생활 투명 객체 3D 데이터/01.데이터/1.Training/1_원천데이터/TS02.zip",
