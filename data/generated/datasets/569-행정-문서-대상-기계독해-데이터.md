...
## Basic Info
- Dataset Key: `569`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 569`
- Collected At: `2026-03-17T08:40:48.933445+00:00`
...
## File Overview
- File count: `24`
- Total known size: `256.9 MB`
- Representative file paths:
  - `016.행정 문서 대상 기계독해 데이터/01.데이터/2.Validation/원천데이터/VS_multiple_choice.zip`
  - `016.행정 문서 대상 기계독해 데이터/01.데이터/2.Validation/원천데이터/VS_span_extraction.zip`
  - `016.행정 문서 대상 기계독해 데이터/01.데이터/2.Validation/원천데이터/VS_span_extraction_how.zip`
  - `016.행정 문서 대상 기계독해 데이터/01.데이터/2.Validation/원천데이터/VS_tableqa.zip`
  - `016.행정 문서 대상 기계독해 데이터/01.데이터/2.Validation/원천데이터/VS_text_entailment.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `ocr/document` 성격의 `text` 자료로 보입니다. 확인 가능한 파일은 24개이며 알려진 총 용량은 256.9 MB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
