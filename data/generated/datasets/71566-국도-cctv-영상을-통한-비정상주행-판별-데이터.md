...
## Basic Info
- Dataset Key: `71566`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71566`
- Collected At: `2026-03-17T08:41:24.597536+00:00`
...
- Representative file paths:
  - `289.국도 CCTV 영상을 통한 비정상주행 판별 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z01`
  - `289.국도 CCTV 영상을 통한 비정상주행 판별 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z02`
  - `289.국도 CCTV 영상을 통한 비정상주행 판별 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z03`
  - `289.국도 CCTV 영상을 통한 비정상주행 판별 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z04`
  - `289.국도 CCTV 영상을 통한 비정상주행 판별 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z05`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 23개이며 알려진 총 용량은 1.94 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
{
  "dataset_key": 71566,
  "title": "국도 CCTV 영상을 통한 비정상주행 판별 데이터",
  "tags": [
    "computer-vision",
    "video",
    "영상",
  ...
  "file_count": 23,
  "total_size_bytes": 2133170585600,
  "human_size": "1.94 TB",
  "sample_file_paths": [
    "289.국도 CCTV 영상을 통한 비정상주행 판별 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z01",
    "289.국도 CCTV 영상을 통한 비정상주행 판별 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS.z02",
