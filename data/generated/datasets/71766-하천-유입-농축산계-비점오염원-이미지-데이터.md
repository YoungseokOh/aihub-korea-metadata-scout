...
## Basic Info
- Dataset Key: `71766`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71766`
- Collected At: `2026-03-17T08:41:37.848867+00:00`
...
## File Overview
- File count: `20`
- Total known size: `63.8 GB`
- Representative file paths:
  - `092.하천 유입 농축산계 비점오염원 이미지 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_01.한강.zip`
  - `092.하천 유입 농축산계 비점오염원 이미지 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_02.낙동강.zip`
  - `092.하천 유입 농축산계 비점오염원 이미지 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_03.금강.zip`
  - `092.하천 유입 농축산계 비점오염원 이미지 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_04.영산강.zip`
  - `092.하천 유입 농축산계 비점오염원 이미지 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_05.새만금.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 20개이며 알려진 총 용량은 63.8 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
{
  "dataset_key": 71766,
  "title": "하천 유입 농축산계 비점오염원 이미지 데이터",
  "tags": [
    "computer-vision",
    "image",
    "이미지",
