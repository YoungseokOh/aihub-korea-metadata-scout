...
## Basic Info
- Dataset Key: `82`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 82`
- Collected At: `2026-03-17T08:40:25.231524+00:00`
...
## File Overview
- File count: `49`
- Total known size: `836.1 GB`
- Representative file paths:
  - `092.한국인 감정인식을 위한 복합 영상 데이터/01.데이터/1.Training/원천데이터/상처/EMOIMG_상처_TRAIN_04.zip`
  - `092.한국인 감정인식을 위한 복합 영상 데이터/01.데이터/1.Training/원천데이터/상처/EMOIMG_상처_TRAIN_03.zip`
  - `092.한국인 감정인식을 위한 복합 영상 데이터/01.데이터/1.Training/원천데이터/상처/EMOIMG_상처_TRAIN_02.zip`
  - `092.한국인 감정인식을 위한 복합 영상 데이터/01.데이터/1.Training/원천데이터/상처/EMOIMG_상처_TRAIN_01.zip`
  - `092.한국인 감정인식을 위한 복합 영상 데이터/01.데이터/1.Training/원천데이터/불안/EMOIMG_불안_TRAIN_04.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 49개이며 알려진 총 용량은 836.1 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
