...
## Basic Info
- Dataset Key: `612`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 612`
- Collected At: `2026-03-17T08:40:53.371452+00:00`
...
## File Overview
- File count: `484`
- Total known size: `2.66 TB`
- Representative file paths:
  - `180.동의보감 약초 이미지 AI데이터/01.데이터/2.Validation/라벨링데이터/VL_참취.zip`
  - `180.동의보감 약초 이미지 AI데이터/01.데이터/2.Validation/라벨링데이터/VL_천선과나무.zip`
  - `180.동의보감 약초 이미지 AI데이터/01.데이터/2.Validation/라벨링데이터/VL_큰꿩의비름.zip`
  - `180.동의보감 약초 이미지 AI데이터/01.데이터/2.Validation/라벨링데이터/VL_큰뱀무.zip`
  - `180.동의보감 약초 이미지 AI데이터/01.데이터/2.Validation/라벨링데이터/VL_털별꽃아재비.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 484개이며 알려진 총 용량은 2.66 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 484,
  "total_size_bytes": 2926308136960,
  "human_size": "2.66 TB",
  "sample_file_paths": [
    "180.동의보감 약초 이미지 AI데이터/01.데이터/2.Validation/라벨링데이터/VL_참취.zip",
    "180.동의보감 약초 이미지 AI데이터/01.데이터/2.Validation/라벨링데이터/VL_천선과나무.zip",
