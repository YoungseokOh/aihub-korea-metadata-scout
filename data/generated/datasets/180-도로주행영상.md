...
## Basic Info
- Dataset Key: `180`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 180`
- Collected At: `2026-03-17T08:40:35.539130+00:00`
...
## File Overview
- File count: `18`
- Total known size: `277.8 GB`
- Representative file paths:
  - `016.도로주행영상/1.Training/라벨링데이터_0107/bb/도심로.zip`
  - `016.도로주행영상/1.Training/라벨링데이터_0107/bb/자동차전용도로.zip`
  - `016.도로주행영상/1.Training/라벨링데이터_0107/fs/도심로.zip`
  - `016.도로주행영상/1.Training/라벨링데이터_0107/fs/자동차전용도로.zip`
  - `016.도로주행영상/1.Training/라벨링데이터_0107/pg/도심로.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 18개이며 알려진 총 용량은 277.8 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
