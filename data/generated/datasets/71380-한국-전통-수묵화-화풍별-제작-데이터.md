...
## Basic Info
- Dataset Key: `71380`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71380`
- Collected At: `2026-03-17T08:41:07.301383+00:00`
...
## File Overview
- File count: `54`
- Total known size: `1.30 TB`
- Representative file paths:
  - `168.한국 전통 수묵화 화풍별 제작 데이터/01-1.정식개방데이터/Other/제작영상_1. 풍경_(1) 실사_1.zip`
  - `168.한국 전통 수묵화 화풍별 제작 데이터/01-1.정식개방데이터/Other/제작영상_1. 풍경_(1) 실사_2.zip`
  - `168.한국 전통 수묵화 화풍별 제작 데이터/01-1.정식개방데이터/Other/제작영상_1. 풍경_(1) 실사_3.zip`
  - `168.한국 전통 수묵화 화풍별 제작 데이터/01-1.정식개방데이터/Other/제작영상_1. 풍경_(1) 실사_4.zip`
  - `168.한국 전통 수묵화 화풍별 제작 데이터/01-1.정식개방데이터/Other/제작영상_1. 풍경_(1) 실사_5.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 54개이며 알려진 총 용량은 1.30 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 54,
  "total_size_bytes": 1424460336128,
  "human_size": "1.30 TB",
  "sample_file_paths": [
    "168.한국 전통 수묵화 화풍별 제작 데이터/01-1.정식개방데이터/Other/제작영상_1. 풍경_(1) 실사_1.zip",
    "168.한국 전통 수묵화 화풍별 제작 데이터/01-1.정식개방데이터/Other/제작영상_1. 풍경_(1) 실사_2.zip",
