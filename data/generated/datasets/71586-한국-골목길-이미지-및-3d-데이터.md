...
## Basic Info
- Dataset Key: `71586`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71586`
- Collected At: `2026-03-17T08:41:26.406694+00:00`
...
- Representative file paths:
  - `211-1.한국 골목길 이미지 및 3D 데이터/01-1.정식개방데이터/Other/Other.z01`
  - `211-1.한국 골목길 이미지 및 3D 데이터/01-1.정식개방데이터/Other/Other.z02`
  - `211-1.한국 골목길 이미지 및 3D 데이터/01-1.정식개방데이터/Other/Other.z03`
  - `211-1.한국 골목길 이미지 및 3D 데이터/01-1.정식개방데이터/Other/Other.z04`
  - `211-1.한국 골목길 이미지 및 3D 데이터/01-1.정식개방데이터/Other/Other.z05`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `3d/lidar` 자료로 보입니다. 확인 가능한 파일은 15개이며 알려진 총 용량은 1.20 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 15,
  "total_size_bytes": 1322849927168,
  "human_size": "1.20 TB",
  "sample_file_paths": [
    "211-1.한국 골목길 이미지 및 3D 데이터/01-1.정식개방데이터/Other/Other.z01",
    "211-1.한국 골목길 이미지 및 3D 데이터/01-1.정식개방데이터/Other/Other.z02",
