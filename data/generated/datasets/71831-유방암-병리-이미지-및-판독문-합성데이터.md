...
## Basic Info
- Dataset Key: `71831`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71831`
- Collected At: `2026-03-17T08:41:43.001503+00:00`
...
## File Overview
- File count: `13`
- Total known size: `59.2 GB`
- Representative file paths:
  - `11.유방암 병리 이미지 및 판독문 합성데이터/3.개방데이터/1.데이터/Other/Other.zip`
  - `11.유방암 병리 이미지 및 판독문 합성데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_상피내암.zip`
  - `11.유방암 병리 이미지 및 판독문 합성데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_정상 유방조직.zip`
  - `11.유방암 병리 이미지 및 판독문 합성데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_침윤암.zip`
  - `11.유방암 병리 이미지 및 판독문 합성데이터/3.개방데이터/1.데이터/Training/02.라벨링데이터/TL_상피내암.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 13개이며 알려진 총 용량은 59.2 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
