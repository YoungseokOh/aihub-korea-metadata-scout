...
## Basic Info
- Dataset Key: `71300`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71300`
- Collected At: `2026-03-17T08:40:59.388376+00:00`
...
## File Overview
- File count: `48`
- Total known size: `126.4 GB`
- Representative file paths:
  - `024.OCR 데이터(교육)/01-1.정식개방데이터/Training/01.원천데이터/TS_OCR(edu)_E_G1.zip`
  - `024.OCR 데이터(교육)/01-1.정식개방데이터/Training/01.원천데이터/TS_OCR(edu)_E_G2.zip`
  - `024.OCR 데이터(교육)/01-1.정식개방데이터/Training/01.원천데이터/TS_OCR(edu)_E_G3.zip`
  - `024.OCR 데이터(교육)/01-1.정식개방데이터/Training/01.원천데이터/TS_OCR(edu)_E_G4.zip`
  - `024.OCR 데이터(교육)/01-1.정식개방데이터/Training/01.원천데이터/TS_OCR(edu)_E_G5.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `ocr/document` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 48개이며 알려진 총 용량은 126.4 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
