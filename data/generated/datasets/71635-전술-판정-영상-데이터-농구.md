...
## Basic Info
- Dataset Key: `71635`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71635`
- Collected At: `2026-03-17T08:41:29.912467+00:00`
...
## File Overview
- File count: `1087`
- Total known size: `214.3 GB`
- Representative file paths:
  - `115.전술 판정 영상 데이터(농구)/01-1.정식개방데이터/Training/01.원천데이터/TS_1. 동영상_1.zip`
  - `115.전술 판정 영상 데이터(농구)/01-1.정식개방데이터/Training/01.원천데이터/TS_1. 동영상_2.zip`
  - `115.전술 판정 영상 데이터(농구)/01-1.정식개방데이터/Training/01.원천데이터/TS_2.이미지.zip`
  - `115.전술 판정 영상 데이터(농구)/01-1.정식개방데이터/Training/02.라벨링데이터/TL_1.동영상분석_000342_01.zip`
  - `115.전술 판정 영상 데이터(농구)/01-1.정식개방데이터/Training/02.라벨링데이터/TL_1.동영상분석_000342_03.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 1087개이며 알려진 총 용량은 214.3 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
