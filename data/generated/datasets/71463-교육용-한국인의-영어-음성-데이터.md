...
## Basic Info
- Dataset Key: `71463`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71463`
- Collected At: `2026-03-17T08:41:14.134544+00:00`
...
## File Overview
- File count: `120`
- Total known size: `78.7 GB`
- Representative file paths:
  - `037.교육용 한국인의 영어 음성 데이터/01-1.정식개방데이터/Training/02.라벨링데이터/TL_SPK_en_ED_IS.zip`
  - `037.교육용 한국인의 영어 음성 데이터/01-1.정식개방데이터/Training/02.라벨링데이터/TL_SPK_en_ED_LC.zip`
  - `037.교육용 한국인의 영어 음성 데이터/01-1.정식개방데이터/Training/02.라벨링데이터/TL_SPK_en_ED_OB.zip`
  - `037.교육용 한국인의 영어 음성 데이터/01-1.정식개방데이터/Training/02.라벨링데이터/TL_SPK_en_ED_OP.zip`
  - `037.교육용 한국인의 영어 음성 데이터/01-1.정식개방데이터/Training/02.라벨링데이터/TL_SPK_en_ED_PR.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `speech/dialogue` 성격의 `audio` 자료로 보입니다. 확인 가능한 파일은 120개이며 알려진 총 용량은 78.7 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
