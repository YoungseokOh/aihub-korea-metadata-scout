...
## Basic Info
- Dataset Key: `216`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 216`
- Collected At: `2026-03-17T08:40:38.674245+00:00`
...
- Representative file paths:
  - `148.한국인 두피 상태 이미지/01.데이터/1.Training/라벨링데이터/3.모낭사이홍반/0.양호/0.양호.zip`
  - `148.한국인 두피 상태 이미지/01.데이터/1.Training/라벨링데이터/3.모낭사이홍반/1.경증/1.경증.zip`
  - `148.한국인 두피 상태 이미지/01.데이터/1.Training/라벨링데이터/3.모낭사이홍반/2.중등도/2.중등도.zip`
  - `148.한국인 두피 상태 이미지/01.데이터/1.Training/라벨링데이터/3.모낭사이홍반/3.중증/3.중증.zip`
  - `148.한국인 두피 상태 이미지/01.데이터/1.Training/라벨링데이터/4.모낭홍반농포/0.양호/0.양호.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 97개이며 알려진 총 용량은 50.1 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
