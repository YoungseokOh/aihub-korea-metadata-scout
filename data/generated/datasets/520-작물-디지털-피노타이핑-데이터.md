...
## Basic Info
- Dataset Key: `520`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 520`
- Collected At: `2026-03-17T08:40:45.116980+00:00`
...
## File Overview
- File count: `93`
- Total known size: `1.59 TB`
- Representative file paths:
  - `105.작물 디지털 피노타이핑 데이터/01.데이터/1.Training/라벨링데이터/TL5_SvIR_KIST_bean.zip`
  - `105.작물 디지털 피노타이핑 데이터/01.데이터/1.Training/라벨링데이터/TL6_SvRGBL_KAERI_wheat-1.zip`
  - `105.작물 디지털 피노타이핑 데이터/01.데이터/1.Training/라벨링데이터/TL6_SvRGBL_KAERI_wheat-2.zip`
  - `105.작물 디지털 피노타이핑 데이터/01.데이터/1.Training/라벨링데이터/TL6_SvRGBL_KAERI_wheat-3.zip`
  - `105.작물 디지털 피노타이핑 데이터/01.데이터/1.Training/라벨링데이터/TL6_SvRGBL_KAERI_wheat-4.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 93개이며 알려진 총 용량은 1.59 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "modality_guess": null,
  "file_count": 93,
  "total_size_bytes": 1753308001280,
  "human_size": "1.59 TB",
  "sample_file_paths": [
    "105.작물 디지털 피노타이핑 데이터/01.데이터/1.Training/라벨링데이터/TL5_SvIR_KIST_bean.zip",
    "105.작물 디지털 피노타이핑 데이터/01.데이터/1.Training/라벨링데이터/TL6_SvRGBL_KAERI_wheat-1.zip",
