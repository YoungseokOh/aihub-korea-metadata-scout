...
## Basic Info
- Dataset Key: `52`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 52`
- Collected At: `2026-03-17T08:40:20.795130+00:00`
...
## File Overview
- File count: `220`
- Total known size: `41.3 GB`
- Representative file paths:
  - `096.K-pop 안무 영상 데이터/01.데이터/1.Training/01. 라벨링데이터_0828_ADD/Dream girl (샤이니).zip`
  - `096.K-pop 안무 영상 데이터/01.데이터/1.Training/01. 라벨링데이터_0828_ADD/Dumb Dumb (레드벨벳).zip`
  - `096.K-pop 안무 영상 데이터/01.데이터/1.Training/01. 라벨링데이터_0828_ADD/Gee (소녀시대).zip`
  - `096.K-pop 안무 영상 데이터/01.데이터/1.Training/01. 라벨링데이터_0828_ADD/Good thing (NCT127).zip`
  - `096.K-pop 안무 영상 데이터/01.데이터/1.Training/01. 라벨링데이터_0828_ADD/Halla(태티서).zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 220개이며 알려진 총 용량은 41.3 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
