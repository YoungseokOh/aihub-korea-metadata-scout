...
## Basic Info
- Dataset Key: `237`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 237`
- Collected At: `2026-03-17T08:40:39.380660+00:00`
...
## File Overview
- File count: `481`
- Total known size: `7.47 TB`
- Representative file paths:
  - `069.시설 작물 개체 영상/01.데이터/1.Training/라벨링데이터/파프리카(코니컬)/파프리카_코니컬4.tar`
  - `069.시설 작물 개체 영상/01.데이터/1.Training/라벨링데이터/파프리카(코니컬)/파프리카_코니컬1.tar`
  - `069.시설 작물 개체 영상/01.데이터/1.Training/라벨링데이터/파프리카(코니컬)/파프리카_코니컬2.tar`
  - `069.시설 작물 개체 영상/01.데이터/1.Training/라벨링데이터/파프리카(코니컬)/파프리카_코니컬3.tar`
  - `069.시설 작물 개체 영상/01.데이터/1.Training/라벨링데이터/가지/가지.tar`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 481개이며 알려진 총 용량은 7.47 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 481,
  "total_size_bytes": 8209952669696,
  "human_size": "7.47 TB",
  "sample_file_paths": [
    "069.시설 작물 개체 영상/01.데이터/1.Training/라벨링데이터/파프리카(코니컬)/파프리카_코니컬4.tar",
    "069.시설 작물 개체 영상/01.데이터/1.Training/라벨링데이터/파프리카(코니컬)/파프리카_코니컬1.tar",
