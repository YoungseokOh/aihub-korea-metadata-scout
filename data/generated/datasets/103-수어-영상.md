...
## Basic Info
- Dataset Key: `103`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 103`
- Collected At: `2026-03-17T08:40:27.157011+00:00`
...
## File Overview
- File count: `124`
- Total known size: `2.62 TB`
- Representative file paths:
  - `004.수어영상/1.Training/원천데이터/REAL/SEN/02_real_sen_video.zip`
  - `004.수어영상/1.Training/원천데이터/REAL/SEN/03_real_sen_video.zip`
  - `004.수어영상/1.Training/원천데이터/REAL/SEN/04_real_sen_video.zip`
  - `004.수어영상/1.Training/원천데이터/REAL/SEN/05_real_sen_video.zip`
  - `004.수어영상/1.Training/원천데이터/REAL/SEN/06_real_sen_video.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `speech/dialogue` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 124개이며 알려진 총 용량은 2.62 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 124,
  "total_size_bytes": 2880903743488,
  "human_size": "2.62 TB",
  "sample_file_paths": [
    "004.수어영상/1.Training/원천데이터/REAL/SEN/02_real_sen_video.zip",
    "004.수어영상/1.Training/원천데이터/REAL/SEN/03_real_sen_video.zip",
