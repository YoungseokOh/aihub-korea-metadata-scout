...
## Basic Info
- Dataset Key: `71415`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71415`
- Collected At: `2026-03-17T08:41:09.616382+00:00`
...
## File Overview
- File count: `1808`
- Total known size: `76.8 GB`
- Representative file paths:
  - `118.안면 인식 에이징(aging) 이미지 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_0001.zip`
  - `118.안면 인식 에이징(aging) 이미지 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_0002.zip`
  - `118.안면 인식 에이징(aging) 이미지 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_0003.zip`
  - `118.안면 인식 에이징(aging) 이미지 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_0004.zip`
  - `118.안면 인식 에이징(aging) 이미지 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_0005.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 1808개이며 알려진 총 용량은 76.8 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
