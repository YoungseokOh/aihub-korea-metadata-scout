...
## Basic Info
- Dataset Key: `525`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 525`
- Collected At: `2026-03-17T08:40:45.488114+00:00`
...
- Representative file paths:
  - `104.식물 병 유발 통합 데이터/01.데이터/2.Validation/환경센서/VE4_오이.zip`
  - `104.식물 병 유발 통합 데이터/01.데이터/2.Validation/환경센서/VE5_토마토.zip`
  - `104.식물 병 유발 통합 데이터/01.데이터/2.Validation/환경센서/VE6_파프리카.zip`
  - `104.식물 병 유발 통합 데이터/01.데이터/2.Validation/환경센서/VE1_고추.zip`
  - `104.식물 병 유발 통합 데이터/01.데이터/2.Validation/환경센서/VE2_딸기.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 78개이며 알려진 총 용량은 1.80 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "modality_guess": null,
  "file_count": 78,
  "total_size_bytes": 1975342071808,
  "human_size": "1.80 TB",
  "sample_file_paths": [
    "104.식물 병 유발 통합 데이터/01.데이터/2.Validation/환경센서/VE4_오이.zip",
    "104.식물 병 유발 통합 데이터/01.데이터/2.Validation/환경센서/VE5_토마토.zip",
