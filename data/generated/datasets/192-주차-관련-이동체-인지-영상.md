...
## Basic Info
- Dataset Key: `192`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 192`
- Collected At: `2026-03-17T08:40:37.387416+00:00`
...
## File Overview
- File count: `183`
- Total known size: `1.94 TB`
- Representative file paths:
  - `058.주차 관련 이동체 인지 영상/01.데이터/2.Validation/1.라벨링데이터/바운딩박스/세종특별자치시/세종특별자치시_FLRR_2_02.zip`
  - `058.주차 관련 이동체 인지 영상/01.데이터/2.Validation/1.라벨링데이터/바운딩박스/세종특별자치시/세종특별자치시_FLRR_2_01.zip`
  - `058.주차 관련 이동체 인지 영상/01.데이터/2.Validation/1.라벨링데이터/바운딩박스/전라북도/전라북도_FLRR_2_01.zip`
  - `058.주차 관련 이동체 인지 영상/01.데이터/2.Validation/1.라벨링데이터/바운딩박스/전라북도/전라북도_FLRR_2_02.zip`
  - `058.주차 관련 이동체 인지 영상/01.데이터/2.Validation/1.라벨링데이터/바운딩박스/전라북도/전라북도_FLRR_2_03.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `video` 자료로 보입니다. 확인 가능한 파일은 183개이며 알려진 총 용량은 1.94 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 183,
  "total_size_bytes": 2138098446336,
  "human_size": "1.94 TB",
  "sample_file_paths": [
    "058.주차 관련 이동체 인지 영상/01.데이터/2.Validation/1.라벨링데이터/바운딩박스/세종특별자치시/세종특별자치시_FLRR_2_02.zip",
    "058.주차 관련 이동체 인지 영상/01.데이터/2.Validation/1.라벨링데이터/바운딩박스/세종특별자치시/세종특별자치시_FLRR_2_01.zip",
