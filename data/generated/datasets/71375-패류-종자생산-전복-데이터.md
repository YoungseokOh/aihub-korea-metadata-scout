...
## Basic Info
- Dataset Key: `71375`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71375`
- Collected At: `2026-03-17T08:41:06.859931+00:00`
...
## File Overview
- File count: `38`
- Total known size: `460.2 GB`
- Representative file paths:
  - `113.패류 종자생산(전복) 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_1.type01a_1.zip`
  - `113.패류 종자생산(전복) 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_1.type01a_2.zip`
  - `113.패류 종자생산(전복) 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_2.type01b.zip`
  - `113.패류 종자생산(전복) 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_3.type02a.zip`
  - `113.패류 종자생산(전복) 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_4.type02b.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 38개이며 알려진 총 용량은 460.2 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
