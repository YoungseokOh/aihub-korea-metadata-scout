...
## Basic Info
- Dataset Key: `576`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 576`
- Collected At: `2026-03-17T08:40:49.708003+00:00`
...
## File Overview
- File count: `200`
- Total known size: `3.64 TB`
- Representative file paths:
  - `166.약품식별 인공지능 개발을 위한 경구약제 이미지 데이터/01.데이터/1.Training/라벨링데이터/경구약제조합 5000종/TL_1_조합.zip`
  - `166.약품식별 인공지능 개발을 위한 경구약제 이미지 데이터/01.데이터/1.Training/라벨링데이터/경구약제조합 5000종/TL_2_조합.zip`
  - `166.약품식별 인공지능 개발을 위한 경구약제 이미지 데이터/01.데이터/1.Training/라벨링데이터/경구약제조합 5000종/TL_3_조합.zip`
  - `166.약품식별 인공지능 개발을 위한 경구약제 이미지 데이터/01.데이터/1.Training/라벨링데이터/경구약제조합 5000종/TL_4_조합.zip`
  - `166.약품식별 인공지능 개발을 위한 경구약제 이미지 데이터/01.데이터/1.Training/라벨링데이터/경구약제조합 5000종/TL_5_조합.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 200개이며 알려진 총 용량은 3.64 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "file_count": 200,
  "total_size_bytes": 4005998624768,
  "human_size": "3.64 TB",
  "sample_file_paths": [
    "166.약품식별 인공지능 개발을 위한 경구약제 이미지 데이터/01.데이터/1.Training/라벨링데이터/경구약제조합 5000종/TL_1_조합.zip",
    "166.약품식별 인공지능 개발을 위한 경구약제 이미지 데이터/01.데이터/1.Training/라벨링데이터/경구약제조합 5000종/TL_2_조합.zip",
