...
## Basic Info
- Dataset Key: `71313`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71313`
- Collected At: `2026-03-17T08:41:00.613335+00:00`
...
## File Overview
- File count: `502`
- Total known size: `13.6 TB`
- Representative file paths:
  - `215.요가 동작 데이터/01-1.정식개방데이터/Training_1/나마스카라사나/나마스카라사나1_1.zip`
  - `215.요가 동작 데이터/01-1.정식개방데이터/Training_1/나마스카라사나/나마스카라사나1_2.zip`
  - `215.요가 동작 데이터/01-1.정식개방데이터/Training_1/나마스카라사나/나마스카라사나1_3.zip`
  - `215.요가 동작 데이터/01-1.정식개방데이터/Training_1/나바사나/나바사나1_1.zip`
  - `215.요가 동작 데이터/01-1.정식개방데이터/Training_1/나바사나/나바사나1_2.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 502개이며 알려진 총 용량은 13.6 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "modality_guess": null,
  "file_count": 502,
  "total_size_bytes": 15000622071808,
  "human_size": "13.6 TB",
  "sample_file_paths": [
    "215.요가 동작 데이터/01-1.정식개방데이터/Training_1/나마스카라사나/나마스카라사나1_1.zip",
    "215.요가 동작 데이터/01-1.정식개방데이터/Training_1/나마스카라사나/나마스카라사나1_2.zip",
