...
## Basic Info
- Dataset Key: `147`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 147`
- Collected At: `2026-03-17T08:40:31.775758+00:00`
...
## File Overview
- File count: `126`
- Total known size: `606.9 GB`
- Representative file paths:
  - `073.노지 작물 질병 진단/01.데이터/2.Validation/원천데이터/07.잎마름병(토마토)/1.질병.zip`
  - `073.노지 작물 질병 진단/01.데이터/2.Validation/원천데이터/07.잎마름병(토마토)/9.증강.zip`
  - `073.노지 작물 질병 진단/01.데이터/2.Validation/원천데이터/07.잎마름병(토마토)/0.정상.zip`
  - `073.노지 작물 질병 진단/01.데이터/2.Validation/원천데이터/08.콩/0.정상.zip`
  - `073.노지 작물 질병 진단/01.데이터/2.Validation/원천데이터/08.콩/1.질병.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 126개이며 알려진 총 용량은 606.9 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
