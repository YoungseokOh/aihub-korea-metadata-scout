...
## Basic Info
- Dataset Key: `71258`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71258`
- Collected At: `2026-03-17T08:40:56.524948+00:00`
...
- Representative file paths:
  - `086.특이 도로 환경 주행 데이터/01.데이터/1.Training/라벨링데이터/TL1.1920x1080.zip`
  - `086.특이 도로 환경 주행 데이터/01.데이터/1.Training/라벨링데이터/TL10.1920x1080.zip`
  - `086.특이 도로 환경 주행 데이터/01.데이터/1.Training/라벨링데이터/TL11.1920x1080.zip`
  - `086.특이 도로 환경 주행 데이터/01.데이터/1.Training/라벨링데이터/TL12.1920x1080.zip`
  - `086.특이 도로 환경 주행 데이터/01.데이터/1.Training/라벨링데이터/TL13.1920x1080.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 30개이며 알려진 총 용량은 1.02 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "modality_guess": null,
  "file_count": 30,
  "total_size_bytes": 1124284235776,
  "human_size": "1.02 TB",
  "sample_file_paths": [
    "086.특이 도로 환경 주행 데이터/01.데이터/1.Training/라벨링데이터/TL1.1920x1080.zip",
    "086.특이 도로 환경 주행 데이터/01.데이터/1.Training/라벨링데이터/TL10.1920x1080.zip",
