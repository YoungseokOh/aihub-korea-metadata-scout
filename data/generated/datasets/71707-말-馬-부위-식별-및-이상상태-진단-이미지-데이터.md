...
## Basic Info
- Dataset Key: `71707`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71707`
- Collected At: `2026-03-17T08:41:33.589851+00:00`
...
- Representative file paths:
  - `069.말 부위 식별 및 이상상태 진단 이미지 데이터/3.개방데이터/1.데이터/Other/Other.zip`
  - `069.말 부위 식별 및 이상상태 진단 이미지 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_HORSE_HOOF_BACK.zip`
  - `069.말 부위 식별 및 이상상태 진단 이미지 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_HORSE_HOOF_BOTTOM.zip`
  - `069.말 부위 식별 및 이상상태 진단 이미지 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_HORSE_HOOF_FRONT.zip`
  - `069.말 부위 식별 및 이상상태 진단 이미지 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_HORSE_HOOF_SIDE.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `computer-vision` 성격의 `image` 자료로 보입니다. 확인 가능한 파일은 70개이며 알려진 총 용량은 2.29 TB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
{
  "dataset_key": 71707,
  "title": "말(馬) 부위 식별 및 이상상태 진단 이미지 데이터",
  "tags": [
    "computer-vision",
    "image",
    "이미지",
  ...
  "file_count": 70,
  "total_size_bytes": 2516512039936,
  "human_size": "2.29 TB",
  "sample_file_paths": [
    "069.말 부위 식별 및 이상상태 진단 이미지 데이터/3.개방데이터/1.데이터/Other/Other.zip",
    "069.말 부위 식별 및 이상상태 진단 이미지 데이터/3.개방데이터/1.데이터/Training/01.원천데이터/TS_HORSE_HOOF_BACK.zip",
