...
## Basic Info
- Dataset Key: `71560`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71560`
- Collected At: `2026-03-17T08:41:24.054983+00:00`
...
## File Overview
- File count: `24`
- Total known size: `44.6 MB`
- Representative file paths:
  - `143.인터페이스(자판-음성)별 고빈도 오류 교정 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_띄어쓰기문장부호오류.zip`
  - `143.인터페이스(자판-음성)별 고빈도 오류 교정 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_맞춤법오류.zip`
  - `143.인터페이스(자판-음성)별 고빈도 오류 교정 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_오탈자.zip`
  - `143.인터페이스(자판-음성)별 고빈도 오류 교정 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_음성인식기오류.zip`
  - `143.인터페이스(자판-음성)별 고빈도 오류 교정 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_자동생성오류.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `speech/dialogue` 성격의 `audio` 자료로 보입니다. 확인 가능한 파일은 24개이며 알려진 총 용량은 44.6 MB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
{
  "dataset_key": 71560,
  "title": "인터페이스(자판/음성)별 고빈도 오류 교정 데이터",
  "tags": [
    "speech/dialogue",
    "audio",
    "음성",
