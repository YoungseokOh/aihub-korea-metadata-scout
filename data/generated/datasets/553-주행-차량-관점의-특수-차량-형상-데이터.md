...
## Basic Info
- Dataset Key: `553`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 553`
- Collected At: `2026-03-17T08:40:47.665753+00:00`
...
## File Overview
- File count: `72`
- Total known size: `1004.0 GB`
- Representative file paths:
  - `090.주행 차량 관점의 특수 차량 형상 데이터/01.데이터/2.Validation/원천데이터_0906_add/VS4.zip`
  - `090.주행 차량 관점의 특수 차량 형상 데이터/01.데이터/2.Validation/원천데이터_0906_add/VS1.zip`
  - `090.주행 차량 관점의 특수 차량 형상 데이터/01.데이터/2.Validation/원천데이터_0906_add/VS2.zip`
  - `090.주행 차량 관점의 특수 차량 형상 데이터/01.데이터/2.Validation/원천데이터_0906_add/VS3.zip`
  - `090.주행 차량 관점의 특수 차량 형상 데이터/01.데이터/2.Validation/원천데이터_0906_add/VS5.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `범용 데이터` 성격의 `명확하지 않은 형태` 자료로 보입니다. 확인 가능한 파일은 72개이며 알려진 총 용량은 1004.0 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
```json
{
  ...
  "modality_guess": null,
  "file_count": 72,
  "total_size_bytes": 1078042034176,
  "human_size": "1004.0 GB",
  "sample_file_paths": [
    "090.주행 차량 관점의 특수 차량 형상 데이터/01.데이터/2.Validation/원천데이터_0906_add/VS4.zip",
    "090.주행 차량 관점의 특수 차량 형상 데이터/01.데이터/2.Validation/원천데이터_0906_add/VS1.zip",
    "090.주행 차량 관점의 특수 차량 형상 데이터/01.데이터/2.Validation/원천데이터_0906_add/VS2.zip",
