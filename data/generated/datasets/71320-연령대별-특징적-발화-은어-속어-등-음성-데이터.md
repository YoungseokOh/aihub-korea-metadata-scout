...
## Basic Info
- Dataset Key: `71320`
- Source Command: `/root/.local/bin/aihubshell -mode l -datasetkey 71320`
- Collected At: `2026-03-17T08:41:01.589498+00:00`
...
## File Overview
- File count: `732`
- Total known size: `540.1 GB`
- Representative file paths:
  - `132.연령대별 특징적 발화(은어·속어 등) 음성 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_대화_10대_직장.zip`
  - `132.연령대별 특징적 발화(은어·속어 등) 음성 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_단어_10대_SNS.zip`
  - `132.연령대별 특징적 발화(은어·속어 등) 음성 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_단어_10대_가족.zip`
  - `132.연령대별 특징적 발화(은어·속어 등) 음성 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_단어_10대_게임.zip`
  - `132.연령대별 특징적 발화(은어·속어 등) 음성 데이터/01-1.정식개방데이터/Training/01.원천데이터/TS_단어_10대_경제.zip`
- Notices / warnings:
  - 공지사항은 보이지 않았습니다.

## What This Dataset Appears To Be
제목과 파일 트리를 기준으로 보면 이 데이터셋은 `speech/dialogue` 성격의 `audio` 자료로 보입니다. 확인 가능한 파일은 732개이며 알려진 총 용량은 540.1 GB입니다. training/validation 분리가 보여서 기본 실험 파이프라인을 구성하기는 쉬워 보입니다. 세부 의미 해석은 하지 않았고, 이 설명은 제목과 파일 트리만으로 추론했습니다 (inferred from title and file tree only).
...
{
  "dataset_key": 71320,
  "title": "연령대별 특징적 발화(은어·속어 등) 음성 데이터",
  "tags": [
    "speech/dialogue",
    "audio",
    "education",
